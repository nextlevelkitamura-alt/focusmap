...
- モバイルの `取り込み` タブでは、下部トレイのチャットカードを長押しするとカードをリフトし、シートを下端の小さなトレイへ縮めてマップを広く見せる。ドラッグ中はノードに近づくと対象ノードを強調し、dropで既存taskの `project_id` と `parent_task_id` を現在マップへ更新して、そのチャットを対象ノードの子ノードとして配置する。drop後はチャットカードをトレイから消し、マップ上では通常のノード形状として表示する。titleは取り込み時のCodex thread titleを優先し、初回取り込み時にtitleが未生成なら本文1行目で仮置きし、後続monitorが `threads.title` を取得できた時だけ未編集タイトルを後追い更新する。markdown本文は `tasks.memo` に保持し、取り込み一覧カードの本文には `## 初回依頼` のプロンプト本文を表示し、ノード上には短い本文プレビューとCodex状態バッジだけを出す。新規にCodex threadを取り込む時は、`tasks.memo` に `Thread ID`、`Repository`、最終更新、初回依頼、最新プレビューをMarkdownで保存し、`tasks.codex_thread_id`、`tasks.codex_work_dir`、紐づく `ai_tasks.result.codex_thread_url` / `codex_run_state` も保持する。長押しD&Dに失敗する端末やアクセシビリティ用に、カード詳細から `配置先を選ぶ` を開き、次にタップしたノードへ配置する代替導線も用意する。
