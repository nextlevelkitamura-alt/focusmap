...
- ノード詳細のCodex同期は、開いているtaskだけ `/api/codex/sync-node` を約3秒で呼び、未送信から実行中/確認待ちへの変化を拾う。Turso mirrorは状態変化、thread検出、thread `updated_at_ms` 変化、current_step変化の時だけ行い、無変化の3秒pollをTurso writeにしない。
