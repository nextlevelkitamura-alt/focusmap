...
- チャット取り込み詳細ヘッダーは、Codex thread titleを主見出しにし、状態チップは見出し右側へ置く。戻る操作は枠なしの矢印アイコンだけにする。repo/project名・配置状態・更新時刻・`Codexで開く` は見出し下のメタ行にまとめる。その下の `AI要約` は枠なしの薄い行UIにし、左にラベル、右に「何をやったか / 何を変えたか / 今の状態 / 次に確認すること」を1〜2行の文章で表示する。AI要約は右端の小さなchevronで折りたため、3カラムや説明文は出さない。AI要約は保存済みactivityと取り込みsnippetから生成し、activity同期中も空欄にしない。
