...
- モバイル `チャット` は全画面の会話面を正にし、上部は `Focusmap` やMac接続状態を出さず、左寄せの小さな `新しいチャット` / `プロジェクトチャット` タグだけを主要表示にする。左端の常設取っ手は置かず、左端から右へのスワイプまたは上部の履歴アイコンでチャット履歴シートを開く。履歴シートを開いた直後は検索入力へ自動フォーカスせず、スマホのキーボードを勝手に出さない。履歴シートは `チャット`、`プロジェクト`、`最近` の順で構成し、`ライブラリ` は置かない。`プロジェクト` はヘッダーで選んだスペース内のプロジェクトを表示し、タップするとそのプロジェクト専用チャットへ切り替えてシートを閉じる。履歴シートの `プロジェクト` と `最近` はテキスト中心のリストにし、フォルダーアイコンや右端の遷移矢印を置かない。会話本文ではアシスタントアイコンを出さず、応答待ちは `考え中` のテキストだけで表示して本文幅を広く使う。履歴は現在のチャットスコープ内で更新日時の新しい順に扱う。通常表示は最近3件だけを見せ、`もっと表示する` / `表示を減らす` で履歴の展開状態を切り替える。新規チャットは上部ヘッダーと履歴シート下部から作成し、スマホでは作成直後に入力欄へ自動フォーカスしない。下部ナビはダッシュボードレイアウト側で維持し、チャット入力欄と重ならない位置に収める。
