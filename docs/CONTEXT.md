...
- チャット取り込み詳細ヘッダーは、Codex thread titleを主見出しにし、状態チップは見出し右側へ置く。戻る操作は枠なしの矢印アイコンだけにする。repo名・配置状態・更新時刻・`Codexで開く` は見出し下のメタ行にまとめ、project名チップは一覧カードや詳細ヘッダーの主要表示へ出さない。その下の `AI要約` は戻るボタン列に依存しない全幅の薄い行UIにし、開いた状態では `ここで何をやったのか` / `次に確認すること` / `変更` の3行を常に表示する。AI要約は右端の小さなchevronで折りたため、閉じた状態では `次に確認すること` だけを1行表示する。AI要約は保存済みactivityと取り込みsnippetから生成し、activity同期中も空欄にしない。Focusmap Macアプリでは `Codexで開く` が `openExternal` bridge 経由で該当 `codex://threads/<thread id>` をOSへ渡し、通常ブラウザでは同じURLのリンクとして開く。
- チャット取り込み一覧カードと詳細ヘッダーの時刻は、初回取り込みや `tasks.updated_at` ではなく、直近のユーザー/Codex可視activityの `created_at` を優先し、`5分前` 形式で表示する。補正は詳細を開いた時と、Mac online時に先頭20件だけ一度行うバックグラウンド同期時に `/api/ai-tasks/[id]/activity` の最大50件から行い、`status` / `system` / pulseログは時刻対象から除外する。activity未取得時だけtask/progress/Codex runの更新時刻をfallbackにする。
...
- `/api/ai-tasks/[id]/activity` は `limit`（既定100、最大200）と `before_created_at` / `before_id` cursorでページングし、Turso/Supabaseとも `created_at DESC, id DESC` を基準に古い履歴へ辿る。チャット取り込みサイドバーの一覧時刻更新は最新1ページだけ取得し、詳細表示を開いた時だけ最大50ページまでcursorを追って全履歴を表示する。Turso `progress_json` にはraw `thread_full_history` を入れず、activity message由来の本文だけを8,000文字上限で読む。
