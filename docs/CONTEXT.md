...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・空き時間検索・マップ整理・ノート整理・タスク化・調査などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。入力欄に固定の `実行` ピルは出さず、追加メニュー・音声・送信だけに絞る。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合し、チャットを開いた時は履歴サイドバーを初期表示する。ヘッダーではスペース切替だけを出してプロジェクト切替は出さない。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース切替だけを置き、同じ場所に重複したブランドやチャットタイトルを出さない。
