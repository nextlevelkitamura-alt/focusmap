...
- スマホ判定時は `codex://` ではなくChatGPTアプリを直接狙う。iOS/iPadOSは `com.openai.chat://codex/open`、Android Chromeは `intent://codex/open#Intent;scheme=com.openai.chat;package=com.openai.chatgpt;end` を使う。`https://chatgpt.com/codex/mobile/` はUniversal/App Linkとしてアプリに渡る場合もあるが、ブラウザに残る環境があるため、Focusmapの主導線では使わない。外部Webから任意のCodex画面へプロンプトを注入する公開deep linkは確認できないため、Focusmapは「コピー済み → ChatGPTアプリ/Codex入口を開く」までを保証範囲にする。
