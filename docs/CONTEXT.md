...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・タスク化・調査などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合し、チャットを開いた時は履歴サイドバーを初期表示する。チャットはプロジェクト選択で絞り込まず、ヘッダーではスペース切替だけを出してプロジェクト切替は出さない。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース切替だけを置き、同じ場所に重複したブランドやチャットタイトルを出さない。
- モバイル `チャット` は全画面の会話面を正にし、左端スワイプまたは左端の取っ手でチャット履歴シートを開く。履歴シートは `チャット`、`プロジェクト`、`最近` の順で構成し、`ライブラリ` は置かない。`プロジェクト` はヘッダーで選んだスペース内のプロジェクトを表示し、ダッシュボード内チャットでは選択中プロジェクトを切り替えられる。履歴は更新日時の新しい順で全チャット履歴を扱う。通常表示は最近3件だけを見せ、`もっと表示する` / `表示を減らす` で履歴の展開状態を切り替える。通常チャット画面でも入力欄の上、右下に新規チャットFABを出し、下部ナビ・safe area・入力欄と重ならない位置へ置く。
