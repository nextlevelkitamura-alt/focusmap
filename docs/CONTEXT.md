...
- Mac/デスクトップの標準導線は、Focusmap MacアプリがNext 3001、`focusmap-agent`、Codex app-server、Codex監視loopをまとめて起動・確認できる時は `dispatch_mode='auto'` を優先する。manual handoffは、スマホ、ローカル未起動、cwd不明、Codex app-server未接続、clipboard失敗、または明示手動時のfallbackとして維持する。
- マインドマップのメモ編集パネル（`CodexNodePanel`）では、「Codexに送る」から同じ手動ハンドオフを実行する。ユーザー本文だけのpromptで `ai_tasks` を作り、プロンプト本文はdeep link queryへ載せない。Mac/デスクトップではtracking task作成後にpromptなしの `codex://?path=...&originUrl=...` またはローカルAPIでCodex.appを開く。スマホではOSの外部アプリ起動制約を優先し、タップ直後に同期クリップボードコピーを開始してからChatGPT Codex入口を即時起動し、保存と `dispatch_mode='manual'` の `ai_tasks` 登録は同時に走らせる。スマホUIの主ボタン名は開始前・未送信詳細とも `Codexを開く` にする。通常ブラウザ/Cloudflareプレビューでは `href="https://chatgpt.com/codex/mobile/"` のアンカーで通常タップ遷移を生かし、Focusmap iOSアプリ内WebViewではpromptがあれば `focusmap:copyText` を送り、その後 `focusmap:openExternal` でReact Native側の `Linking.openURL(url)` に渡す。スマホの起動先はiOS/iPadOSでは `https://chatgpt.com/codex/mobile/` のUniversal Link、Androidでは `intent://chatgpt.com/codex/mobile/#Intent;scheme=https;package=com.openai.chatgpt;S.browser_fallback_url=https%3A%2F%2Fchatgpt.com%2Fcodex%2Fmobile%2F;end` を使う。
...
- Codex送信後、`未送信` / `prompt_waiting` / thread未検出 / 外部アプリ起動失敗など、まだ `実行中` へ進んでいない状態では、メモ詳細・ノード詳細・リンクメモ詳細・Codex看板詳細に `Codexを開く` とprompt再コピーのボタンを残す。スマホの `Codexを開く` は同じタップで端末クリップボードコピーを開始し、公式Codex mobile URLへ進む。Focusmap iOSアプリ内では同じボタンがWebView bridgeで外部URLを開くため、既に `focusmap:openExternal` を受け取れるアプリなら外部起動はCloud RunのWeb更新だけで反映される。ネイティブclipboard保証は `focusmap:copyText` を受け取る新しいアプリ本体が必要で、古いアプリではWebView側clipboard fallbackだけが効く。React Native側のbridge、native clipboard、bundle URL自体を変えた場合、または古いインストール済みアプリにbridgeが無い場合は、USB接続して `mobile/focusmap-app/scripts/install-ios-free.sh` で再インストールする。`実行中` になったら通常は再コピー導線を隠し、以後はCodex.app側のthreadを正にする。
- localhost、`*.local`、`*.trycloudflare.com` のローカル/スマホプレビューでは、デスクトップの場合だけローカルAPI `/api/codex/open-repo` からMacの `pbcopy`、`open codex://...`、Codex.appのactivateを実行する。スマホ判定時はMac側のローカルAPIへ送らず、端末側クリップボードへの同期コピーとChatGPT mobile入口を優先する。ブラウザが外部アプリ起動を止めても、プロンプトはクリップボードに残し、Focusmap側は `ai_tasks` の `未送信` として状態を追う。
...
- `Codexに送る` 直後のmanual handoffは `未送信` として表示する。Codex.appのthreadを検出しても、Codex側で送信・活動が確認できるまでは `未送信` のままにする。threadの `updated_at_ms` や `last_activity_at` が直近の3秒UI pollで動いていれば `実行中`、Mac heartbeatはあるがCodex activityが止まっていれば停止疑いまたは `確認待ち` とする。ユーザー可視の出力本文やpreviewは通常snapshotへ保存しない。
...
- Mac側の監視間隔はactive task中だけ1〜5秒まで許容する。ただしDB書き込みはhash、活動時刻、状態、新規activityが変わった時だけにし、heartbeatは5秒基準にする。UIの3秒pollはreadだけなのでDB write量を増やさない。
...
- Supabase Auth制限で開発が止まる場合に限り、ローカル開発用の非常口として `FOCUSMAP_DEV_AUTH=1` と `FOCUSMAP_DEV_USER_ID=<uuid>` を使える。これは `localhost` / `127.0.0.1` / `*.localhost` かつ `NODE_ENV !== production` の時だけ有効で、middleware・サーバー/ブラウザSupabase client・`authenticateSupabaseRequest` が同じdev userを返す。`SUPABASE_JWT_SECRET` / `SUPABASE_AUTH_JWT_SECRET` がある場合はローカルでHS256 Supabase JWTも署名し、Supabase Authを呼ばずにRLS付きDBリクエストへ `Authorization: Bearer` を付ける。JWT secretが未設定で `SUPABASE_SERVICE_ROLE_KEY` がローカルにある場合、サーバー側 `createClient()` だけservice role keyでDBを読む。service role keyはブラウザへ返さず、本番/Cloud Runへローカルdev auth系envを設定しない。
...
- iPhoneアプリの WebView では `mediaCapturePermissionGrantType="grantIfSameHostElsePrompt"` を指定し、同一ホストのマイク許可を再利用する。Expo `app.json` の `ios.infoPlist` には `NSMicrophoneUsageDescription` / `NSCameraUsageDescription` / `NSPhotoLibraryUsageDescription` / `NSPhotoLibraryAddUsageDescription` を日本語で設定し、画像選択やマイク許可のシステム表示が日本語ロケールに寄るよう `CFBundleDevelopmentRegion` / `CFBundleLocalizations` を `ja` にする。生成済み `mobile/focusmap-app/ios` はgit管理外なので、ソース上の正は `app.json` と `App.tsx`。
...
- iPhoneアプリ内の外部URL起動は、WebView側が `focusmap:openExternal` messageを送るか、React Native側の `onShouldStartLoadWithRequest` が非HTTP/外部認証URLを検出した時に `Linking.openURL(url)` でOSへ渡す。Codex mobile起動もこのbridgeを使い、WebView内で `chatgpt.com` を表示し続けない。`focusmap:copyText` messageは `expo-clipboard` で端末クリップボードへ書き込む。
