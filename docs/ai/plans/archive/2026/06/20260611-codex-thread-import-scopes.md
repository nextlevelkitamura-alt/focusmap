...
- `projects.repo_path` を変更/解除した場合は取り込みをOFFに戻し、変更後のrepoは再度ONにした時刻以降だけ取り込む。
...
- `focusmap-agent` のSQLite検索をscope repoの `cwd` に絞り、scan件数と1tick import上限を分離
...
- `npm run test:run -- 'src/app/api/projects/[id]/route.test.ts' scripts/focusmap-agent/codex-thread-monitor.test.ts`
