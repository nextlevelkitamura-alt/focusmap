    ...
    return (
        <div ref={panelRef} className="h-full flex flex-col bg-background/50 backdrop-blur-sm border-l border-border/30 relative overflow-hidden">
            ...
            {logic.unscheduledTasks.length > 0 && (
                <div className="flex-shrink-0 border-t border-border/30 px-3 py-1.5">
                    <p className="text-[10px] font-medium text-muted-foreground mb-1">未スケジュール</p>
                    <div className="max-h-24 overflow-y-auto space-y-0.5">
                        {logic.unscheduledTasks.map(task => (
                            <div key={task.id} className="flex items-center gap-2 py-1 px-1 rounded-md hover:bg-muted/50 transition-colors">
                                <button
