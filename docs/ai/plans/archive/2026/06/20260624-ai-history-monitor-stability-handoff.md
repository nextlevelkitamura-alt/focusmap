...
- AGENTS.md
- docs/CONTEXT.md
- docs/ai/plans/archive/2026/06/20260624-ai-history-monitor-stability-handoff.md
- scripts/focusmap-agent/src/codex-thread-monitor.ts
- scripts/focusmap-agent/src/cli.ts
- scripts/focusmap-agent/codex-thread-monitor.test.ts
...
- Worktree: `/Users/kitamuranaohiro/Private/focusmap-main-ai-history-integration`
- Base: local `main` commit `8bf65cd5c475a8931586c76f45f005a84644e678`; `origin/main` was not used as the start point.
- Reviewed local-main commits:
  - `5c0e8d01` Phase 1 Agent Hot Path
  - `4e8472e4` Phase 2A Backend/API Snapshot Contract
  - `8bf65cd5` Phase 2B Frontend 2s snapshot poll / detail burst
- Scope: Agent / API / UI 3秒反映経路の契約一致レビュー、最小統合修正、docs/board更新。
- Push/deploy: not run.
- Mac app rebuild/install: not run by instruction.

Evaluation:

- Agent:
  - `startCodexThreadMonitorLoop` は `runActiveWatch` / `refreshTargets` / `runRecentScan` / `runDetailHydrate` / `runReconcile` を分離しており、1秒active watchへreconcile/backfill/detail hydrate/all rollout parseは戻っていない。
  - heartbeat metadataで `codex_last_tick_duration_ms`, `codex_skipped_ticks`, `codex_tick_overrun_ms`, `codex_phase_timings_ms`, `codex_active_watch_count`, `codex_recent_scan_count`, `codex_reconcile_queue_length` を観測できる。
  - running durationは分単位bucketのhashに含める既存抑制を維持し、毎秒Turso writeを追加していない。
  - `RESUME_RUNNING_VISIBILITY_MS` はUIの2秒snapshot pollで見逃しにくい `6_000` msへ延長した。
- API:
  - `/api/ai-history/snapshot` はmetadata-onlyのままで、counts / reconcile / detail hydrate request / raw本文を呼ばない。
  - snapshot policyは `countsIncluded:false`, `reconcileIncluded:false`, `detailHydrateRequestsCreated:false`, `rawBodiesIncluded:false` を返す。
  - `/api/ai-history/[id]/activity?watch=1` はTTL refresh付きhydrate requestとして冪等。通常pollは既存requestが未期限切れなら毎回writeしない。
- UI:
  - 初回/フィルタ変更は `/api/ai-history`、2秒pollは `/api/ai-history/snapshot?include_deleted=true`。
  - snapshot itemは既存itemsへmergeし、`deletedAt` / archivedも扱う。countsは2秒ごとに再取得しない。
  - detail pollは選択中1件だけ。open直後は約10秒の1秒burst、その後は5秒側へ戻る。
  - `selectedChatItem` のobject identity変化でdetail hydrate pollが再スケジュールされ続けるリスクを、選択ID基準のref/keyとhydrate継続判定で止めた。
  - interval / timeout / AbortController cleanupは、既存のcleanupと今回のstable key化で維持している。

Integration fixes:

- Agent resume running可視windowを2秒から6秒へ延長し、2秒snapshot pollの境界でrunning表示を取りこぼすリスクを下げた。
- resolver versionを `2026-06-24-ai-history-stability-v2` へ更新し、source agentの状態を識別可能にした。
- Sidebar detail hydrate pollを「選択中1件、かつレスポンス上まだhydrate requiredの時だけ継続」に変更し、hydrated後の不要なself-loopを止めた。
- Sidebar testのfake timersを実時間前進付きにし、同じpoll契約を安定検証できるようにした。
- `docs/CONTEXT.md` のresume running可視window記述を約6秒へ揃えた。

Verification:

- `npm run test:run -- scripts/focusmap-agent/codex-thread-monitor.test.ts --test-timeout=30000`: passed, 55 tests.
- `npm run test:run -- scripts/focusmap-agent/codex-thread-monitor.test.ts src/hooks/useAiHistory.test.ts src/components/dashboard/codex-chat-import-sidebar.test.tsx src/app/api/ai-history/snapshot/route.test.ts 'src/app/api/ai-history/[id]/activity/route.test.ts' src/app/api/agents/ai-history/active-monitor-targets/route.test.ts --test-timeout=30000`: passed, 6 files / 85 tests.
- `npm --prefix scripts/focusmap-agent run build`: passed.
- `npx eslint scripts/focusmap-agent/src/codex-thread-monitor.ts scripts/focusmap-agent/src/cli.ts src/hooks/useAiHistory.ts src/components/dashboard/codex-chat-import-sidebar.tsx src/app/api/ai-history/snapshot/route.ts 'src/app/api/ai-history/[id]/activity/route.ts' src/app/api/agents/ai-history/active-monitor-targets/route.ts`: passed.
- `git diff --check`: passed.
...
- 厳密な3秒以内は、Codex file write timing、agent tick phase、Turso/Next API latency、browser visibility timer throttlingの合算で変動するため、runtime計測は未完了。
- 今回はMac app rebuild/installを実行していない。source agent buildは通っているが、既存 `/Applications/Focusmap.app` にこのresolver versionが入っているとは限らない。
- 本番反映・pushは未実行。local mainでのsource統合までが完了範囲。
...
[router 状態] 目的: AI履歴監視の詰まり解消
 詰め: 完了  分解: 4タスク  実装: 完了
 #1 Agent Hot Path      local main統合済み  完了
 #2 Backend/API         local main統合済み  完了
 #3 Frontend/UI         local main統合済み  完了
 #4 Integration/Review  最終評価  完了
 検証: 完了   main統合: 完了   push/deploy: 未
```
