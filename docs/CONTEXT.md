...
- チャット取り込み詳細ヘッダーは、Codex thread titleを主見出しにし、状態チップは見出し右側へ置く。戻る操作は枠なしの矢印アイコンだけにする。repo名・配置状態・更新時刻・`Codexで開く` は見出し下のメタ行にまとめ、project名チップは一覧カードや詳細ヘッダーの主要表示へ出さない。その下の `AI要約` は戻るボタン列に依存しない全幅の薄い行UIにし、「何をやったか / 何を変えたか / 今の状態 / 次に確認すること」を文章で表示する。AI要約は右端の小さなchevronで折りたため、開いた状態では生成済み文言を内部で省略せず、閉じた状態では1行だけを表示し、3カラムや説明文は出さない。AI要約は保存済みactivityと取り込みsnippetから生成し、activity同期中も空欄にしない。Focusmap Macアプリでは `Codexで開く` が `openExternal` bridge 経由で該当 `codex://threads/<thread id>` をOSへ渡し、通常ブラウザでは同じURLのリンクとして開く。
...
- 実行中はMac側で Codex state DB / rollout JSONL / app-server通知を1秒基準で確認してよい。`focusmap-agent` のCodex monitor loopは通常1秒tickだが、監視対象taskと取り込みscopeのAPI再取得は10秒基準にし、scope取得後の直接開始thread検出は同じ1秒tick内でローカルsqliteを読む。看板・一覧向けのWeb UIは3秒pollを基準にし、Turso snapshotは `runner_heartbeats.current_task_id`、`last_seen_at`、`ai_tasks.last_activity_at/current_step/summary` の短い状態だけを読む。Mac `focusmap-agent` の通常flushは2秒最短だが、hashまたは活動時刻が変わった時と状態変化時だけにし、同一hashや同じ活動時刻なら送信しない。thread検出・running再開・確認待ち・完了・失敗など状態変化だけforce送信する。これにより、リポ監視ON直後やagent再起動直後の新規チャット反映は10秒scope refresh + UI 3秒pollの影響を受けうるが、scope取得済みならローカル検出はおおむね次の1秒tickで走り、DB書き込みは変化時だけに絞る。
