...
### Phase 1: ポップオーバーの作成
- [ ] `calendar-event-popover.tsx` を新規作成
- [ ] Radix UI Popover で実装
- [ ] イベント情報の表示（タイトル、時間、カレンダー名、優先度、通知）
- [ ] 編集ボタン → フルモーダルへの導線
- [ ] 削除ボタン → 確認ダイアログ → 削除処理

### Phase 2: フル編集モーダルの作成
- [ ] `calendar-event-edit-modal.tsx` を更新
- [ ] タイトル入力
- [ ] 時間セレクター（15分刻み、ドロップダウン）
- [ ] 日付入力
- [ ] 優先度セレクト
- [ ] カレンダー種別セレクト（色付きドット）
- [ ] 保存・キャンセルボタン
...
### Phase 3: 既存コードの統合
- [ ] カレンダーイベントカードのクリックハンドラを変更
- [ ] `prompt()` 呼び出しを完全に削除
- [ ] ポップオーバー → モーダルの状態遷移を管理
- [ ] PATCH API呼び出し + カレンダー再フェッチ

### Phase 4: UIポリッシュ
- [ ] ローディングスピナー（保存中）
- [ ] バリデーション（終了時刻 > 開始時刻、タイトル必須）
- [ ] キーボード対応（Esc で閉じる）
- [ ] アニメーション（フェードイン/アウト）
- [ ] ダークモード対応
