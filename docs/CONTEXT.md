...
- 実装上の正本は、DBが `mindmap_drafts` / `mindmap_draft_nodes` / `mindmap_draft_history`、APIが `/api/mindmap/drafts`、`/api/mindmap/drafts/[draftId]/nodes`、`/api/mindmap/drafts/[draftId]/apply`、`/api/mindmap/draft-history/[historyId]/undo|redo`。AIツールは `saveMindmapDraft` を下書き保存専用に使い、`proposeMindmapOrganization` は読み取り専用にする。永続プロジェクトチャットの実行では `agent_chat_sessions.id` を `saveMindmapDraft` へ自動注入し、下書きの `chat_session_id` に保存することで、確定/Undo/Redo後の短い完了メッセージを同じ `マップチャット` へ戻す。マップUIは `mindmap_drafts` / `mindmap_draft_nodes` のRealtimeと `focusmap:mindmap-draft:changed` イベントでactive draftを再読込し、AI案上の手動追加・移動・タイトル変更は下書きノードAPIへ保存する。確定時に `useUndoRedo` へDB履歴ベースのUndo/Redo actionを登録し、Cmd/Ctrl+Z系の既存導線へ接続する。
