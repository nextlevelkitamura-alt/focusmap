...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、空の新規チャットでは会話面の中央に ChatGPT 風スターターとして `マインドマップを整理` / `AI実行を整理` / `メモから作成` / `リポジトリを読む` を出す。スターターは会話開始前だけの入口であり、送信後の会話中やキーボード上、入力欄左の `+` メニュー内に常設しない。各スターターは、対象がプロジェクトチャットならプロジェクト概要・蓄積コンテキスト・マップ見出しを読んだうえで、まず会話で整理案を返し、新規追加したい項目や他の移動候補をユーザーが返せる流れにする。入力欄左の `+` メニューは写真添付など補助操作に絞り、固定の `実行` ピルは出さず、追加・モデルモード・音声・送信だけに絞る。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合し、チャットを開いた時は履歴サイドバーを初期表示する。ヘッダーではスペース切替だけを出してプロジェクト切替は出さない。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース切替だけを置き、同じ場所に重複したブランドやチャットタイトルを出さない。
