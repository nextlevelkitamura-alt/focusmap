...
export function buildMindmapOrganizationHarness(): MindmapOrganizationHarness {
  return {
    rules: [
      ...
      'まず構造診断を行い、「今のままでよい」「軽く整理」「構造見直し推奨」のどれかを判断する。ある程度整理されている場合は、無理に新規グループや移動案を作らない。',
      '整理不要または効果が薄い場合は saveMindmapDraft を呼ばず、現在構造を維持する理由、追加するとよさそうな項目、深掘り質問を返す。',
      '構造変更に明確な価値がある場合だけ、本番DBへ直接反映せず saveMindmapDraft で AI案 下書きとして保存する。大きく範囲を広げる時だけ先に確認する。',
      'チャット経由の整理結果は addMindmapGroup / moveMindmapNode / updateMindmapNode で本番DBへ直接反映しない。必要な場合だけ saveMindmapDraft で AI案 下書きへ保存する。',
      '判断に迷う候補だけ getMindmapNodeDetail や getNoteOrganizationDetail で詳細を読む。',
      '削除は提案しない。まず新規まとめノード作成、既存ノード配下への移動、必要なら名称変更案だけに絞る。',
      '追加するとよさそうな新規項目は、ユーザーが「入れて」と承認するまでAI案へ自動保存しない。候補名、理由、入れるなら自然な親ノードを会話で示す。',
      '提案では、新しいノードを作って紐づける案と、既存ノード配下へ入れる案の両方を検討する。片方が不要な場合は理由を短く添える。',
    ],
    response_format: [
      '1. 採用範囲: 既定では現在マップ上のノードのみ。ユーザーが明示した場合だけ、含めたCodex/メモ/ノート範囲を短く明記する。',
      '2. 構造診断: 「今のままでよい」「軽く整理」「構造見直し推奨」のどれかと理由を短く出す。',
      '3. 読んだ前提: プロジェクト概要、現在の見出し数、見出しの大枠を短く共有する。',
      '4. 整理提案: 変更する価値がある場合だけ、新規ノード作成案、既存ノードへ入れる案、移動するノード、理由、実行操作を出す。整理不要なら「AI案は作成していません」と明記する。',
      '5. 追加候補: AI案へ自動保存せず、追加するとよさそうな項目を1〜5件、理由と入れるなら自然な親ノードつきで出す。',
      '6. 深掘り質問: 次に考えるとよい論点を1〜3件だけ出す。',
      '7. 図: AI案を保存した場合だけ ```text``` のツリーで「現在 → 提案後」を見せる。',
      '8. 次の操作: AI案として保存した場合は、チャットの確定ボタン、または画面の AI案 バーから確定できることだけを短く伝える。',
    ],
    save_draft_rules: [
      '構造診断で「今のままでよい」なら saveMindmapDraft を呼ばない。',
      'saveMindmapDraft に新規ノード、既存ノード移動、ユーザー明示のタイトル調整、元メモ/チャット紐づきだけを渡す。',
      '追加候補や深掘り候補は、ユーザーが追加を承認するまで saveMindmapDraft の nodes に入れない。',
      'AIによる既存タイトル一括変更、メモ/状態/進捗/予定変更、削除候補は下書き保存対象に入れない。',
      '保存後も、追加候補と深掘り質問は短く添える。詳細な表は不要。',
    ],
    diagram_template: [
      '```text',
      '現在',
