...
- メモ編集シートの本文欄は、同じ枠の中を本文スクロール領域と下部の保存状態表示に分ける。PCでは `本文を音声入力` と `本文から見出し生成` をメモ詳細見出し行の右側、コピーアイコンの左側へ小さく並べ、コピーは文言を出さずアイコンだけにする。スマホでは見出し行にキラキラを出さず、本文枠の下部右側に `本文から見出し生成` と `本文を音声入力` のアイコンボタンを隣接配置する。本文が長くなってもスクロールは本文領域だけで発生する。音声入力は `useVoiceRecorder` と `/api/transcribe` を使い、文字起こし結果を本文へ追記する。見出し生成は `/api/ai/generate-memo-heading` を使い、本文から14〜22字程度、長くても24字以内の短い日本語見出しを作る。
...
- モバイルの通常 `メモ` 画面は左上の重複見出しを出さず、スペース/プロジェクト切替を左詰めに置く。看板切替は `メモ` / `今日` / `完了` の3つだけを表示し、予定済み・マップ追加済みのDB状態は `メモ` 側へ集約してカード内チップで示す。ホーム入力列は、本文入力欄の右に `音声入力`、その右に `AIで整理して生成`、一番右に `メモを追加` を置く。音声・AI・追加は同じ四角いoutlineアイコンボタンで揃え、プラスボタンは最右端の純粋追加導線として固定する。AI整理追加の見出しは14〜22字、最大22字に抑え、入力本文はAI要約で削らず原文をメモ本文として保存する。
