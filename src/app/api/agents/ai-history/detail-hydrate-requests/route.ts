...
import {
  listAiHistoryDetailHydrateRequests,
  toAiHistoryDetailHydrateRequestItem,
...
async function assertRunnerCanSync(
  ...
  if (!executors.some(executor => VALID_EXECUTORS.includes(executor as (typeof VALID_EXECUTORS)[number]))) {
    return { ok: false as const, status: 403, error: 'Runner is not allowed to hydrate AI history detail' }
  }
...
async function handleRequest(request: NextRequest, input: { runnerId: string | null; limit: number }) {
  try {
    ...
    const requests = await listAiHistoryDetailHydrateRequests({
    ...
    return NextResponse.json({
      ok: true,
      source: 'turso',
      requests: requests.map(toAiHistoryDetailHydrateRequestItem),
      policy: {
    ...
    console.error('[agents/ai-history detail hydrate requests]', error)
    const message = error instanceof Error ? error.message : 'AI history detail hydrate requests fetch failed'
