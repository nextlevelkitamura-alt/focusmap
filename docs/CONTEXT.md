...
- prompt付きのローカルAPI起動では、テキストだけなら `pbcopy`、先頭画像URLがある場合はmacOS `NSPasteboard` にテキストと画像データを同じpasteboard itemとして保存してからCodex.appを開く。Electron bridgeも `clipboard.write({ text, image })` で同じくテキスト+画像を保存する。日本語promptが文字化けしない形でコピーし、クリップボードコピーに失敗した場合はCodex.appを開かずエラーとして返し、UI側は送信済み扱いにしない。画像取得に失敗した場合はテキストコピーを優先し、複数画像はprompt本文のURL一覧を正として先頭画像だけをクリップボード補助にする。
