...
- マップ画面のCodex監視はチャットtabへ逃がさず、マップ内で完結させる。デスクトップはマップ下に折りたたみ式 `Codex看板` を置き、初期状態は畳んで件数だけを見せ、必要な時だけ展開する。展開時は看板上端のドラッグハンドルを上へ動かすと表示高さを広げられ、保存した高さを次回表示にも使う。モバイルは右下の `Codex` ボタンから下シートで看板を開く。右下ボタン自体にも `online` / `offline` / `確認中` のMac状態を出し、スマホでFocusmap/Codex runnerが動いているかを看板を開かず確認できるようにする。看板の初期スコープは現在表示中のマップだが、看板内のスペース/プロジェクト切替で別プロジェクトのマップノードへ切り替えられる。看板内スコープ切替はトップバーの現在マップ選択を変更しない。看板レーンは `未送信` / `実行中` / `確認待ち` / `接続失敗` / `完了済み`。`未送信` はCodex.appでまだ開始されていないもの、`確認待ち` はCodex出力や完了を人間が確認するものに限定し、同じカード内で両方の意味を併記しない。`完了済み` レーンはノードがチェック済みで、紐づくprogressまたはノード更新日が当日のものだけを一時表示し、翌日以降は看板から消す。
- モバイルCodex看板は、Sheet上部のステータスチップを横スクロールでき、タップまたは横スワイプで `未送信` / `実行中` / `確認待ち` / `接続失敗` / `完了済み` の表示レーンを切り替える。デスクトップ/モバイルのCodex看板は看板で選択中のプロジェクトに紐づく `source_type='mindmap'` snapshotだけを表示し、選択中プロジェクトの `tasks` に `source_id` が残っていないカード、削除済みノード由来のカード、source不明の古いカードは看板から除外する。Turso snapshotに `source_type` / `source_id` が欠けていても、同じ `ai_task.id` が `/api/ai-tasks?source=linked` の看板対象ノード紐付けに残っていれば、看板表示前に `source_type='mindmap'` / `source_id` を補完する。進捗snapshotや linked `ai_tasks` がまだ無いマップノードも、看板では `未送信` カードとして合成表示し、そこからCodexノードパネルを開ける。
- Codex看板カードは `status`、`current_step`、`summary`、Mac online/offline、最終更新を表示する。`source_type='mindmap'` で元ノードが残っているカードには完了チェック、Codex実行ボタン、削除ボタンを出し、チェックは既存ノードチェックと同じ `status=done/todo` 更新、削除は現在マップなら既存ノード削除処理、看板で別プロジェクトを選んでいる場合は全体タスク更新経路へ委譲する。チェック/削除はAPI完了を待たずに看板上へ即時反映し、失敗時だけ元に戻す。補完後に元ノードへ解決できるsnapshotも同じく操作可能にする。モバイルカードは情報量を絞り、状態・今やっていること・確認要否を優先する。進捗があるカードの本文をタップするとTurso progress詳細panel/drawerを右サイドパネルまたはモバイルdrawerとして開き、未送信の合成カード本文またはCodex実行ボタンを押すと `CodexNodePanel` を開いて、その看板対象ノードからmanual handoffを開始できる。
- デスクトップマップ右上のCodexロゴボタンは `チャット取り込み` 右サイドバーを開く。サイドバー表示中は右上のCodex/設定ボタンを隠し、サイドバーはマップ領域の上下右端に隙間なく接地させて下部の `Codex看板` と重ねない。サイドバー内には見出しの `チャット取り込み` 文言を出さず、先頭から監視・リポ選択・チャット一覧を表示して一覧領域を広く取る。サイドバーはproject単位ではなく選択中のリポ単位の未配置Inboxとして扱い、Mac側 `focusmap-agent` が登録した `available_repos` 候補またはFinder選択で対象リポを切り替える。通常UIには長いパス手入力欄や常時表示の候補リストを出さず、`リポ監視` 行に対象リポ名と監視中project名を小さく出し、既存リポ候補は `既存リポ選択` のポップアップ、任意フォルダはFinder導線から選ぶ。`リポ監視` は選択リポに対して有効な `projects.codex_thread_import_enabled` scopeがあるかを表示し、ONにする時は現在projectの `projects.repo_path` を対象リポへ合わせてscopeを作る。OFFにする時は同じリポの有効scopeを止める。サイドバー下部は選択リポの `source='codex_app_thread'` かつ `codex_work_dir` が一致し、どこかの `Codex Inbox` グループ直下に残っている未配置チャットだけを表示する。現在project外に取り込まれた同一リポのチャットも一覧に出し、チャット行を現在マップのノードまたはproject rootへドラッグすると、既存taskの `project_id` と `parent_task_id` を現在マップへ更新して配置する。配置後は未配置一覧から消える。チャット行クリックでその1件のtask詳細だけを `/api/tasks/[id]` から取得して開き、一覧表示のために全文や詳細tailをバックグラウンドで常時取得しない。チャット行の削除は既存taskを削除し、グローバルUndoで削除前task snapshotを復元できる。UIはCodex sqlite/rolloutを直接読まず、取り込み済みノードとtask-progress snapshotの読み取りだけで一覧を作る。配置済みチャットは複製しない。
