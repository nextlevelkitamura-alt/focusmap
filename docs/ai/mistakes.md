...
- Focusmap Macアプリ同梱のCodex監視コードを更新せずに使い続けない。Codex Desktopのstate DBが `~/.codex/sqlite/state_5.sqlite` へ移った後も、古いbundleが旧 `~/.codex/state_5.sqlite` を読むと実在threadを見失い、`thread_deleted` / `awaiting_approval` を誤書きする。Macアプリ、`focusmap-agent`、`/api/codex/sync-node`、互換runnerは同じresolverを使い、bundle更新後は再インストール/再起動を確認する。
