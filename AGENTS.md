...
5. 通常作業はそのまま `git push origin main` まで行い、Cloud Run は `origin/main` から自動デプロイされる前提で扱う
6. 大きい/危険/本番保留の作業だけ、事前にユーザーへ伝えてから `feat/*` / `fix/*` を作る
7. **迷ったらブランチではなく、小さくコミットして戻せる状態にする**
