...
/**
 * モデル選定 (intent判定 = 構造化JSON出力):
 *   - DEEPSEEK_API_KEY 設定時: デフォルトで deepseek-v4-pro を使い、 Thinking モードは明示的に
 *     OFF にすることで generateObject (JSON schema 出力) と両立させる。
 *     (DEEPSEEK_INTENT_MODEL > DEEPSEEK_MODEL > デフォルト deepseek-v4-pro)
 *   - 未設定時: Gemini 3.1 Flash Lite にフォールバック
 *
 * ⚠️ 経緯:
 * - 以前は v4-pro の Thinking デフォルトONが原因で generateObject が落ちたため flash に逃がしていた。
 * - SDK の providerOptions.deepseek.thinking = { type: 'disabled' } で Thinking 自体を切れることが
 *   分かったので、 v4-pro でも安全に構造化出力ができるようになり Pro に戻す。
 * - reasoningEffort: 'low' を併用して intent判定の応答時間を抑える。
 */
function getModel() {
  if (process.env.DEEPSEEK_API_KEY) {
    const modelId =
      process.env.DEEPSEEK_INTENT_MODEL ?? process.env.DEEPSEEK_MODEL ?? 'deepseek-v4-pro';
...
/**
 * deepseek (V4) の Thinking モードは generateObject と相性が悪く JSON 出力が崩れる事例があるため
 * intent 判定では明示的に Thinking を切る。
 */
const DEEPSEEK_INTENT_PROVIDER_OPTIONS = {
  deepseek: {
...
export async function classifyIntent(userMessage: string): Promise<IntentResult> {
  ...
  const result = await generateObject({
    ...
    temperature: 0.2,
    ...(usingDeepseek
      ? { providerOptions: DEEPSEEK_INTENT_PROVIDER_OPTIONS }
      : {}),
  });
...
export function getActiveModelLabel(): string {
  ...
  return 'gemini-3.1-flash-lite (fallback)';
}
