...
- モバイルの通常 `メモ` 画面は左上の重複見出しを出さず、スペース/プロジェクト切替を左詰めに置く。看板切替は `メモ` / `今日` / `完了` の3つだけを表示し、予定済み・マップ追加済みのDB状態は `メモ` 側へ集約してカード内チップで示す。展開前の入力導線はヘッダー下ではなく、下部ナビの上に固定したチャット風バーにする。バーには `話した内容やメモを入力` の本文入力、四角い `音声入力`、四角い `AIで整理して生成`、白い丸の `メモを追加` を横並びで置く。AI整理追加は `/api/ai-ingest` で見出しを作り、入力本文をAI要約で削らず原文のままメモ本文へ入れた保存前の詳細シートを開く。この段階では `/api/wishlist` へPOSTせず、`保存` で作成し、閉じた場合はローカルドラフトを破棄する。追加先は表示中の看板に連動するが、`完了` 表示中は `メモ` へ戻す。
