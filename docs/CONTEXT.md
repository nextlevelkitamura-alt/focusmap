...
- メモ編集シートの本文欄は、同じ枠の中を本文スクロール領域と下部ツールバーに分け、ツールバー内に `本文を音声入力` と `本文から見出し生成` を小さく並べる。本文が長くなってもスクロールは本文領域だけで発生し、音声・見出し生成ボタンは枠内に常時表示する。音声入力は `useVoiceRecorder` と `/api/transcribe` を使い、文字起こし結果を本文へ追記する。見出し生成は `/api/ai/generate-memo-heading` を使い、本文から短い日本語見出しを作る。
