...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・タスク化・調査などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合する。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース/プロジェクト切替を置き、同じ場所に重複したブランドやチャットタイトルを出さない。
- モバイル `チャット` は全画面の会話面を正にし、左端スワイプまたは左端の取っ手でチャット履歴シートを開く。履歴シートは最近の会話一覧を主表示にし、右下の大きい `チャット` ボタンで新規チャットを作る。通常チャット画面でも入力欄の上、右下に新規チャットFABを出し、下部ナビ・safe area・入力欄と重ならない位置へ置く。
