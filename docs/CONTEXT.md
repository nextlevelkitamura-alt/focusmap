...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・空き時間検索・マップ整理・ノート整理・タスク化・調査・メモ一括追加などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。入力欄に固定の `実行` ピルは出さず、追加メニュー・音声・送信だけに絞る。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合し、チャットを開いた時は履歴サイドバーを初期表示する。ヘッダーではスペース切替だけを出してプロジェクト切替は出さない。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース切替だけを置き、同じ場所に重複したブランドやチャットタイトルを出さない。
...
- モバイル `マップ` 画面のヘッダーは `メモ` 画面と同じコンパクトな `SpaceProjectSwitcher` を使う。右上操作は、右端にNetworkアイコンへSparklesを重ねたメモ→マップ生成ボタン、左隣に文字なしのBotアイコンボタンを置く。ルート追加の `+` と単独チャットアイコンは出さない。Sparklesボタンは選択中プロジェクトの未予定メモを `wishlist` から取得し、既存の `MemoToMindmapDialog` を開いてメモからマインドマップを生成・配置する。Botアイコンボタンはマップ内のCodexシートを開き、`取り込み` / `看板` をセグメントで切り替える入口にする。ヘッダー上に `AI` などの文字ラベルは出さない。
