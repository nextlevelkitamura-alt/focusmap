...
Codexチャットをアーカイブした後に、対象ノードをチェック済みにし、Codex看板や詳細UIでは `完了済み` と表示する。Codexの通常実行完了やthread missing/deleteは `確認待ち` に留める。チェックを外した場合は、Codex表示を `確認待ち` に戻す。
...
- `codex_source_task_completed=true` のCodex状態を `完了済み` と表示し、通常の `completed` は `確認待ち` に留めるよう、ノード右上バッジ、Codex看板、進捗詳細、ノード詳細の表示丸めを更新した。
- マップノード/ノード詳細のチェック操作から、紐づく最新Codex taskを `completed` / `awaiting_approval` へ同期するクライアントヘルパーを追加した。
- チェック解除時は `codex_source_task_completion_suppressed=true` を保存し、アーカイブ済みthreadの再同期でノードが自動再チェックされないようにした。
