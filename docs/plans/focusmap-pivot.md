...
Week 4（4/28-5/2）: 仕上げ
- [x] 認証状態表示UI
- [x] 日別の振り返り画面（完了タスク・壁打ち履歴）
- [x] デプロイ（Cloud Run自動デプロイ）
- [ ] ai_tasks テーブル作成（Supabase DBマイグレーション適用待ち）
- [ ] 自分で1週間使ってみて改善
```
...
現行のGit運用は `AGENTS.md` を正とする。初期案では機能ごとに `feat/*` ブランチを切る前提だったが、現在は小さな修正・UI調整・ドキュメント変更は `main` で小さくコミットし、push / deploy は別ゲートで明示依頼時だけ行う。

```
1つの小さな修正を作る流れ:

  git fetch / status / worktree確認
    ↓
  mainで小さく実装・整理
    ↓
  差分確認（自分の変更だけ）
  ...
  コミット
    ↓
  local main / origin/main / 本番反映状態を分けて報告
```

ブランチやworktreeを使うのは、ユーザーが明示した場合、大きな機能、数日またぐ作業、破壊的変更、DBマイグレーション、本番にすぐ出したくない変更、または並列実装で編集範囲を分離できる場合に限る。worktree / feature / fix ブランチにコミットが残っているだけでは完了扱いにせず、原則として `local main` へ取り込んでから完了報告する。

### いつ何を実行するかのガイド

| タイミング | 自動？ | 実行されるもの |
|-----------|:------:|---------------|
| 作業開始時 | 手動 | `git fetch --prune origin`、`git status --short --branch`、`git worktree list` |
| 変更後 | 手動 | `git diff` / `git status` で自分の差分だけ確認 |
| テスト・lint・build・ブラウザ確認 | 明示依頼時のみ | `AGENTS.md` の自動検証ポリシーに従い、勝手に実行しない |
| 機能1つ完成後 | 手動 | `/simplify`（冗長コード最適化） |
| Week の終わり | 手動 | `/quality`（全体品質監査） |
| ビルドエラー時 | 手動 | `/fix`（エラー分析→最小修正） |
| push / deploy 前 | 明示依頼時のみ | `origin/main` 反映やCloud Run本番反映は別ゲートで確認 |

### 安全ガードレール

| ガード | 何を防ぐ |
|--------|---------|
| `AGENTS.md` Gitルール優先 | 古い計画書のfeatureブランチ前提へ戻ること |
| local main完了ゲート | worktree / feature / fix ブランチにだけ成果が残ること |
| push / deploy明示ゲート | 未確認の変更が `origin/main` や本番へ出ること |
| 差分範囲確認 | 既存の未コミット差分や別作業を混ぜること |
