...
スマホでは既存のマイク録音APIを使い、アプリ内で録音した音声を `/api/transcribe` に送って文字起こしする。文字起こし結果は入力欄へ追記し、ユーザーが確認してから `整理/生成` を押す。

推奨UI:

- 1〜3行のコンパクト入力
- 送信ボタン
- マイクボタン、録音中の波形、文字起こし中の状態表示
- 詳細入力は提案レビュー側で編集する
...
OpenAI互換の `chat/completions` を基本形にする。標準は OpenCode Go の `https://opencode.ai/zen/go/v1/chat/completions` と `kimi-k2.6`。OpenCode Go 経由では独自パラメータを避け、`model/messages/max_tokens` の標準パラメータだけを送る。Moonshot直APIを使う場合のみ、Kimi K2.6 のJSON整理用途で `thinking: {"type":"disabled"}` を付ける。
...
音声入力は既存の `src/hooks/useVoiceRecorder.ts` を使い、ブラウザの `MediaRecorder` で録音した音声を `src/app/api/transcribe/route.ts` へ送る。音声認識は `GROQ_API_KEY` が必要。
