...
- 通常 `チャット` と `プロジェクト` チャットは別スコープとして扱う。通常チャットは全スペース/全プロジェクトへアクセスできるが、最初から特定プロジェクト情報を読み込まず、AGENTS.md が無い素のチャットに近い前提で会話する。左サイドバーの `プロジェクト` から入るプロジェクトチャットは `project:<id>` ごとの別履歴を使い、会話面にプロジェクト名と `プロジェクトチャット` ラベルを表示し、入力欄のplaceholderも対象プロジェクト名へ切り替える。`/api/ai/agent` へは `chatMode='project'` と `projectId` を渡して、プロジェクト説明・蓄積コンテキスト・タスク要約を事前にsystem promptへ注入する。
...
- モバイル `チャット` は全画面の会話面を正にし、上部に `Focusmap`、チャット種別、Mac接続状態、履歴、新規チャットを置く。左端スワイプまたは左端の取っ手でチャット履歴シートを開く。取っ手は矢印ではなく小さなグリップ表示にする。履歴シートは `チャット`、`プロジェクト`、`最近` の順で構成し、`ライブラリ` は置かない。`プロジェクト` はヘッダーで選んだスペース内のプロジェクトを表示し、タップするとそのプロジェクト専用チャットへ切り替わる。履歴シートの `プロジェクト` と `最近` はテキスト中心のリストにし、フォルダーアイコンや右端の遷移矢印を置かない。会話本文ではアシスタントアイコンを出さず、応答待ちは `考え中` のテキストだけで表示して本文幅を広く使う。履歴は現在のチャットスコープ内で更新日時の新しい順に扱う。通常表示は最近3件だけを見せ、`もっと表示する` / `表示を減らす` で履歴の展開状態を切り替える。新規チャットは上部ヘッダーと履歴シート下部から作成し、会話面の上には重なる浮動ボタンを置かない。下部ナビはダッシュボードレイアウト側で維持し、チャット入力欄と重ならない位置に収める。
...
- メモ詳細のCodex実行表示は `ai_task_activity_messages` を `/api/ai-tasks/[id]/activity` から読み、チャット風活動メッセージを正とする。詳細を開いた時は `/api/codex/sync-node` に `include_visible_activity=true` を付け、rollout JSONLの `agent_message` / assistant `message` / `task_complete.last_agent_message` / Codex側のユーザー追加入力を複数件dedupeしてactivityへ残す。さらに、Codex task が `running` / `prompt_waiting` から `awaiting_approval` へ初めて入る時と、Codex thread archiveで完了扱いへ進む時は、詳細表示を待たずに同じ可視メッセージを1回先取り保存する。これにより、スマホ側で確認待ち/確認済みになった後に詳細を開いた時、読める範囲の会話内容を即表示できる。`Codex.appの稼働シグナルを確認中` のようなpulse文はチャット本文として表示しない。確認待ちでは最新の質問/承認メッセージを最上部に出し、raw `live_log` は補助データとしてDBに残すだけにする。マップの `CodexNodePanel` とスマホ版メモ編集シートは、スマホで `見出し → メモ詳細 → 画像 → Codex → 時間・予定 → タグ` の順にし、タグを低頻度操作として最下部へ置く。Codex欄は画像の直後、時間・予定より前に置き、送信後は同じ欄で状態とチャットを確認できるようにする。Codexログとチャットはスマホ幅内で折り返し、メッセージ取得後もシート全体を横スクロールさせない。
...
- 自作マップのタスクノード右端の三点ボタンは、小さなノード下メニューを出さず、対象ノードを選択して `CodexNodePanel` のメモ編集UIを直接開く。デスクトップでは右側から出るシート、スマホでは下部シートとして表示し、ノード下へ `Codexを開く` / `日時を指定する` のメニューを重ねない。日時・所要時間・画像・Codex実行はこの編集シート側で扱い、Codex進捗はノード右上の状態バッジからも開ける。
...
- `CodexNodePanel` は `メモを編集` のフル編集パネルとして扱う。デスクトップでは画面右から `w-[min(92vw,460px)]` 程度のシートで開き、スマホでは下部シートで開く。上部は見出しだけを置き、本文側は1カラムで `メモ詳細 → 画像 → Codex → 時間・予定 → タグ` の順に並べる。Codexボタンは画像セクション直後に白黒の `Codexに送る` として置き、保存ボタンは下部に黄緑系の `保存` を固定気味に表示する。スマホの画像追加は「画像を追加」だけを出し、クリップボード貼り付けはPC導線に寄せる。画像は `/api/tasks/[id]/attachments` でフォルダー選択、ドラッグ&ドロップ、クリップボード貼り付け、削除を行う。保存済み画像は添付カード内のコピーアイコンからCodex貼り付け用にコピーでき、カード本体クリックでプレビューを開く。`task_attachments` APIはユーザーセッションで対象タスクと添付の所有確認を行い、その後のStorage操作と添付DB操作はserver-side service role clientで実行する。アップロード中は元画像のローカルプレビューを薄く表示し、オンライン送信前にクライアント側でJPEGへ圧縮して300KB以下にする。保存済み添付に入るまで、またはMac heartbeatが未確認/offlineの間はCodex送信を無効にし、サーバー側も300KB超の画像を拒否する。時刻・所要時間・カレンダーIDは親の `onUpdateTask` 経由で `scheduled_at` / `estimated_time` / `calendar_id` を更新し、日付選択UIは出さない。時刻変更時は既存 `scheduled_at` の日付を保持し、未設定なら当日の日付を内部補完する。所要時間の `カスタム` は時間/分ホイールPopoverで選ぶ。カレンダーは `useCalendars` の選択済み/書き込み可能なGoogleカレンダーをプルダウン表示し、時刻・所要時間・カレンダーが揃った時だけ `予定を登録` / `予定を更新` ボタンから `/api/calendar/sync-task` に登録できる。簡易な `メモ見出し` / `メモ詳細` だけの画面や、三点メニューから別操作を選ぶ導線へ戻さない。
