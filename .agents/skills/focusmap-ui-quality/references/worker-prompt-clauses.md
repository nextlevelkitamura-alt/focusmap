...
広い見た目変更では、承認済みモックアップ画像または明示されたno-image例外を確認してください。P0/P1の止血修正ではモックアップを待たず、最小修正を優先してください。
...
承認済みモックアップ画像がある場合は、レイアウト、密度、階層、状態の視覚ターゲットとして参照してください。画像内のダミー文言を、既存のプロダクト文言より優先しないでください。
