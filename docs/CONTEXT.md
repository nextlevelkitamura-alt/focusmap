...
- マインドマップのメモ編集パネル（`CodexNodePanel`）では、「Codexに送る」から同じ手動ハンドオフを実行する。ユーザー本文だけのpromptで `ai_tasks` を作り、プロンプト本文はdeep link queryへ載せない。Mac/デスクトップではtracking task作成後にpromptなしの `codex://?path=...&originUrl=...` またはローカルAPIでCodex.appを開く。スマホではOSの外部アプリ起動制約を優先し、タップ直後に同期クリップボードコピーを開始してからChatGPT Codex入口を即時起動し、保存と `dispatch_mode='manual'` の `ai_tasks` 登録は同時に走らせる。スマホUIの主ボタン名は開始前・未送信詳細とも `Codexを開く` にする。通常ブラウザ/Cloudflareプレビューでは `href="https://chatgpt.com/codex/mobile/"` のアンカーで通常タップ遷移を生かし、Focusmap iOSアプリ内WebViewでは `window.ReactNativeWebView.postMessage({ type: "focusmap:openExternal", url })` でReact Native側の `Linking.openURL(url)` に渡す。スマホの起動先はiOS/iPadOSでは `https://chatgpt.com/codex/mobile/` のUniversal Link、Androidでは `intent://chatgpt.com/codex/mobile/#Intent;scheme=https;package=com.openai.chatgpt;S.browser_fallback_url=https%3A%2F%2Fchatgpt.com%2Fcodex%2Fmobile%2F;end` を使う。
...
- Codex送信後、`未送信` / `prompt_waiting` / thread未検出 / 外部アプリ起動失敗など、まだ `実行中` へ進んでいない状態では、メモ詳細・ノード詳細・リンクメモ詳細・Codex看板詳細に `Codexを開く` とprompt再コピーのボタンを残す。スマホの `Codexを開く` は同じタップで端末クリップボードコピーを開始し、公式Codex mobile URLへ進む。Focusmap iOSアプリ内では同じボタンがWebView bridgeで外部URLを開くため、既に `focusmap:openExternal` を受け取れるアプリならCloud RunのWeb更新だけで反映される。React Native側のbridgeやbundle URL自体を変えた場合、または古いインストール済みアプリにbridgeが無い場合だけ、USB接続して `mobile/focusmap-app/scripts/install-ios-free.sh` で再インストールする。`実行中` になったら通常は再コピー導線を隠し、以後はCodex.app側のthreadを正にする。
...
- iPhoneアプリ内の外部URL起動は、WebView側が `focusmap:openExternal` messageを送るか、React Native側の `onShouldStartLoadWithRequest` が非HTTP/外部認証URLを検出した時に `Linking.openURL(url)` でOSへ渡す。Codex mobile起動もこのbridgeを使い、WebView内で `chatgpt.com` を表示し続けない。
