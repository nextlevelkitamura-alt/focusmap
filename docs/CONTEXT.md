...
- モバイル `Todo > 予定` の右下プラスから開く新規タスク追加シートと、予定/タスクをタップした時の編集シートは黒背景のボトムシートにし、編集シート上部は中央ハンドルと右上 `完了` だけを置き、`予定を編集` 見出し・左キャンセル・下部保存/削除バーは出さない。閉じる操作は上部ハンドルを下へ引くジェスチャーにし、シートを下方向へ追従させてから閉じる。日時はスマホ標準の `datetime-local` 入力で日付/時刻をまとめて選び、15分刻みを既定にする。通知の未設定既定は15分前、所要時間の未設定既定は15分とし、所要時間/通知は2列で表示する。追加先カレンダーは `useMomentumWheel` の縦ホイールで、上下の候補が複数見える高さ、上下フェード、中央ハイライト、選択行のスケール/透明度差でホイール感を出し、右端矢印は置かない。Google連携済みタスクの編集保存では、`MobileEventEditModal` から `/api/calendar/events/[eventId]` を先にPATCHし、返却された `google_event_id` / `calendar_id` を `localTasks` と `onUpdateTask` へ反映する。カレンダー変更時は `originalCalendarId` / `source_calendar_id` で移動元を渡し、`/api/tasks/[id]` と `/api/calendar/sync-task` は移動先の `calendar_id` と返却された実カレンダーIDをタスクへ保存する。メモはサブタスクの上に2行程度の入力欄として表示し、サブタスクはフォーム最下部に置く。プロジェクトと優先度は表示しない。サブタスクは追加直後にローカル仮行として即時表示し、保存後は `tasks.parent_task_id` で親タスクへ紐づく子タスクとしてDB保存する。チェック状態は `tasks.status` で扱い、右端のゴミ箱で子タスクを削除できる。`所要時間` は通常時は値だけを表示し、タップ時だけ5分/15分/30分等のプリセットとグリッド内のカスタム導線をインライン展開し、時間選択後は所要時間パネルを閉じる。編集シート本体は小画面でフォーム本文だけ縦スクロールし、サブタスクが増えた時だけサブタスク一覧内もスクロールする。
