...
- メモ詳細の `Codexにプロンプトを送る` は `executor='codex_app'` / `dispatch_mode='auto'` の `ai_tasks` を `pending` で作り、Mac runner が `ws://127.0.0.1:7878` のCodex app-serverへ送信する。初期表示では送信ボタンだけを見せ、押下後または進行中タスクがある時だけ再コピー・手動handoff・runner状態・進行ログを展開する。メモ本文はラベルや前置きなしでそのままCodexへ渡す。実行にはプロジェクト/リポジトリ設定が必要。
...
- マインドマップのCodex.app連携主導線は手動ハンドオフ。オンラインrunnerがあっても、ノードパネルの「Codexに送る」は `dispatch_mode='manual'` の `ai_tasks` を作り、メモ見出し本文とメモ詳細本文だけをラベルなしでクリップボードへコピーし、Codex.appを開く。メモ詳細の `Codexで実行` だけは、ローカルMacのrunnerが使える前提で `dispatch_mode='auto'` を正とする。
...
- 実行中は体感優先で短い間隔で追う。launchdの通常起動に加え、実行中は3秒間隔の追加follow-upを最大4回入れる。ローカル/プレビュー上のメモ・マップ表示中も、`codex_run_state='running'` のCodexタスクだけ `/api/codex/sync-node` を約3秒間隔で呼び、確認待ち・完了・失敗は3秒監視しない。
...
- DBにはCodexの全生ログを保存しない。`ai_tasks.result` は `current_step` / `live_log` / `codex_run_state` / `last_activity_at` / `codex_thread_snapshot` を上書き保持し、チャット風の活動記録は `ai_task_activity_messages` に1実行最大50件で保存する。50件超過時は通常進捗から削除し、送信・質問・確認待ち・再開・完了・失敗・ユーザー回答を優先保持する。
...
- Codex監視の軽量データはTursoへ段階移行する。Turso schemaは `db/turso/migrations/20260605000000_codex_monitoring.sql` を正とし、`ai_tasks` / `ai_task_progress` / `ai_task_events` / `runner_heartbeats` / `screenshots` metadataだけを持つ。Next API側のTurso clientは `src/lib/turso/client.ts` に集約し、`TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`（または `LIBSQL_*`）はサーバー環境変数だけで使う。
- `/api/task-progress` はMac agentの軽量progress JSON POSTと、Web/iPhoneのprogress snapshot GETを受ける。Turso未設定時のGETだけ既存Supabase `ai_tasks.result` の最小JSON pathへfallbackし、POSTはTurso必須にする。既存 `/api/ai-tasks/[id]/live-log` と `/api/ai-tasks/[id]/activity` は、Tursoにprogress/eventがある場合はTursoを優先し、古いSupabase activity/resultは互換fallbackとして残す。
- 新規 `/api/ai-tasks` 作成、`/api/agents/tasks/[id]/state`、`/api/agents/heartbeat`、`/api/ai-runners/heartbeat` は、Tursoが設定されている場合だけ軽量dual-writeする。dual-write失敗で既存Supabase導線を落とさない。agent token認証は5分メモリキャッシュし、Heartbeat/Progressで毎回Supabase token lookupへ戻らない。
...
- スマホ/マップUIのCodex進捗表示は `/api/task-progress/snapshot?updated_after=<cursor>&limit=500` を読む。初回はcursorなし、以後は返却された `cursor` 以降の差分だけ取得する。マップ上では `source_type='mindmap'` / `source_id` があればそれを優先し、無い場合は既存 `source_task_id -> ai_task.id` の対応からsnapshot taskをマップノードへ紐づける。ノード上には `running` / `awaiting_approval` / `completed` / `failed` / `needs_input` の短いstatus badgeと、`current_step` または `summary` の短縮表示だけを出す。`running` がある時は5秒poll、AI詳細panel/drawerを開いている時はsnapshotと詳細tailを3秒poll、runningが無い時は45秒pollまたは手動更新にする。詳細panel/drawerだけ `/api/task-progress?task_id=<id>&limit=50` でtail履歴を読み、マップ一覧では詳細ログを読まない。表示確認用に `?taskProgressFixture=1` または `localStorage.focusmap:task-progress-fixture=1` でrunning/completed/failed/awaiting_approvalのfixtureをマップへ重ねられる。Mac agentはCodex.app app-serverのWebSocket通知をローカルで1〜2秒単位に拾うが、Tursoへの通常progress送信は内容hashが変わった時だけ、かつ最短3秒間隔にする。`codex_thread_ready` / `codex_prompt_sent` / `awaiting_approval` / `completed` / `failed` は即時送信する。
- Mac agentから `/api/task-progress` へ送るpayloadは、`result.live_log` / `output` / 生のCodex通知全文を含めない。`current_step` は最新ログ行または最新stepを600文字以内、`summary` は最新ログ行または承認待ち要約を1200文字以内に圧縮し、`progress_json` は `executor` / `codex_run_state` / `codex_thread_id` / `last_activity_at` / 直近8steps / 文字数などのcompact metadataだけにする。これによりTursoの `ai_tasks` は一覧表示用snapshot、`ai_task_progress` は短い履歴、`ai_task_events` は状態変化イベントを持ち、全文ログの保管場所にはしない。
...
- Macアプリ/ローカルWebのメモ詳細では、Codex app-serverのWebSocket自動投入を主導線にする。Electron側は `ANTHROPIC_API_KEY` / `CLAUDECODE` を外した環境で起動し、クリップボードコピー・Codex.app起動・ローカルsqlite/rollout同期も担う。マインドマップのノードパネルやスマホ単体など、app-server自動投入を使えない場面では手動ハンドオフを正とする。
