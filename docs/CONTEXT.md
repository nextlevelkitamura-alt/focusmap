...
- 通常 `チャット` と `プロジェクト` チャットは別スコープとして扱う。通常チャットは全スペース/全プロジェクトへアクセスできるが、最初から特定プロジェクト情報を読み込まず、AGENTS.md が無い素のチャットに近い前提で会話する。左サイドバーの `プロジェクト` から入るプロジェクトチャットは `project:<id>` ごとの別履歴を使い、デスクトップ会話面にはプロジェクト名と `プロジェクトチャット` ラベルを表示する。モバイル会話面ではプロジェクト名ブロックを出さず、上部タグと対象プロジェクト名入りplaceholderだけで文脈を示す。`/api/ai/agent` へは `chatMode='project'` と `projectId` を渡して、プロジェクト説明・蓄積コンテキスト・タスク要約を事前にsystem promptへ注入する。
- `/api/ai/agent` は Codex 風のツール実行エージェントとして扱う。通常会話だけで終わらせず、必要に応じて `listProjects` / `getProjectContext` / `saveProjectContext` / `updateProject` / `listProjectTasks` / `listNotesForOrganization` / `getNoteOrganizationDetail` で Focusmap DB のプロジェクト概要・蓄積コンテキスト・タスク・ノート/メモを確認/記録/更新する。ノート/メモ整理では `listNotesForOrganization` が見出しと詳細冒頭30文字を標準で返し、AIは本文全量を無差別に読まず、判断に必要な候補だけ `getNoteOrganizationDetail` で詳細文字数を増やす。`getMindmapOverview` / `getMindmapNodeDetail` / `updateMindmapNode` / `moveMindmapNode` / `updateMindmapMemoLink` でマインドマップDBのノード一覧、親子関係、進捗、予定情報、メモ紐づき、ノード移動、メモ紐づき付け替えを扱う。`listCalendarEvents` / `findCalendarOpenSlots` / `checkCalendarAvailability` / `addCalendarEvent` / `updateCalendarEvent` で既存予定の確認、複数日の空き枠検索、新規予定作成、見出し・内容・時間変更を行う。Mac が online の時は既存の `runTerminal` / `listFiles` / `readFile` / `writeFile` / `runOpenCode` / browser 系ツールも使える。低リスクな確認・作成・更新は自律実行するが、削除、大量変更、外部公開、送信、課金、権限変更、秘密情報の表示や保存は事前確認し、Mac agent 側の危険コマンドブロックと許可ルートを迂回しない。通常チャットで対象プロジェクト、対象ノード、対象メモ、対象予定が曖昧な時は、検索ツールで候補を出してから確認する。
