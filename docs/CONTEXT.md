...
- `Todo` タブ左側のメモカードは、`メモ` 画面と同じ `WishlistCardDetail` 編集シートを開く。見出し・本文・タグ・画像・予定化などのメモ編集導線は左ペインからも同じ挙動にする。狭幅ブラウザでも `useIsMobile` はモバイル幅判定でスマホUIに切り替える。スマホの通常メモ編集は黒背景の下部シートを正にし、上部ハンドル、`メモを追加` 見出し、右上の `メモ` ピルと×、`見出し`、`メモの内容`、`所要時間`、`画像`、黄緑の `保存` だけを表示する。通常のスマホ編集シートにはCodex、タグ、日付、時刻、カレンダー、構造化パネル、対話履歴を出さない。シートを開いた直後は入力欄へ自動フォーカスせず、キーボードを出さない。スマホでは上端から下へ引いた時も閉じる。
- メモ編集シートの本文欄は、同じ枠の中を本文スクロール領域と下部の保存状態表示に分ける。PCでは `本文を音声入力` と `本文から見出し生成` をメモ詳細見出し行の右側、コピーアイコンの左側へ小さく並べ、コピーは文言を出さずアイコンだけにする。スマホの下部編集シートでは、`見出し` 入力欄の右端にSparklesの見出し生成ボタンを置き、`メモの内容` 欄の右上に本文からの見出し生成、右下に音声入力を置く。本文が長くなってもスクロールは本文領域だけで発生する。音声入力は `useVoiceRecorder` と `/api/transcribe` を使い、文字起こし結果を本文へ追記する。見出し生成は `/api/ai/generate-memo-heading` を使い、本文から14〜22字程度、長くても24字以内の短い日本語見出しを作る。
...
- デスクトップマップ右上のCodexロゴボタンは `チャット取り込み` 右サイドバーを開く。サイドバー上部の `リポ / 構成` では、Mac側 `focusmap-agent` が登録した `available_repos` から候補を表示し、必要ならFinder選択または手入力で `projects.repo_path` を保存する。`リポ監視` は `projects.codex_thread_import_enabled` をON/OFFするiOS設定風スイッチで、repo未設定時はONにできない。サイドバー下部は現在projectの `source='codex_app_thread'` ノードをチャット一覧として表示し、titleはCodex threadの短いsummary/title、memoはfirst user prompt由来のメモを優先する。未配置判定は `Codex Inbox` グループ直下に残っているかで行い、チャット行をマップノードまたはproject rootへドラッグすると、既存のマップ移動処理へ `position='as-child'` で渡して親子関係と並び順を更新する。UIはCodex sqlite/rolloutを直接読まず、取り込み済みノードとtask-progress snapshotの読み取りだけで一覧を作る。
