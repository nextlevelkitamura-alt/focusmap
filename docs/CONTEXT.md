...
- スマホ/マップUIのCodex進捗表示は `/api/task-progress/snapshot?updated_after=<cursor>&limit=500` を3〜5秒間隔で読む。初回はcursorなし、以後は返却された `cursor` 以降の差分だけ取得する。詳細画面だけ `/api/task-progress?task_id=<id>&limit=50` でtail履歴を読む。Mac agentはCodex.app app-serverのWebSocket通知をローカルで1〜2秒単位に拾うが、Tursoへの通常progress送信は内容hashが変わった時だけ、かつ最短3秒間隔にする。`codex_thread_ready` / `codex_prompt_sent` / `awaiting_approval` / `completed` / `failed` は即時送信する。
