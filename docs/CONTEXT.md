...
- モバイルの通常 `メモ` 画面は左上の重複見出しを出さず、スペース/プロジェクト切替を左詰めに置く。看板切替は `メモ` / `今日` / `完了` の3つだけを表示し、予定済み・マップ追加済みのDB状態は `メモ` 側へ集約してカード内チップで示す。展開前の入力導線はヘッダー下ではなく、下部ナビの上に固定したチャット風バーにする。バーは単一の黒いコンパクトな入力面にまとめ、`話した内容やメモを入力` の本文入力、丸い `音声入力`、丸い `メモを追加` を置く。本文が入った時だけSparklesの `AIでメモを予約` ボタンを出す。Sparklesは `/api/ai-ingest` で見出し・日時・所要時間を抽出し、原文を削らず `/api/wishlist` へメモを作成する。日時が抽出できた場合は作成直後に `/api/wishlist/[id]/calendar` へ渡してGoogleカレンダーへ予定登録し、日時が抽出できない場合はメモとして保存して詳細シートを開く。`+ 追加` は従来通り、入力欄に本文がある時は先頭行をタイトルにして即時メモ追加し、空の時は未保存ドラフト詳細を開く。追加先は表示中の看板に連動するが、`完了` 表示中は `メモ` へ戻す。
