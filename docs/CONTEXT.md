...
- Focusmap Macアプリ（Electronシェル）は macOS では `hiddenInset` のネイティブタイトルバーを使い、OS標準の信号ボタンをWebヘッダー内へ溶け込ませる。Web側ヘッダーは `desktop=1` / `source=mac` / `window.focusmapDesktop` を検知した時だけ左に信号ボタン分の余白を取り、Macアプリではヘッダー左上のFocusmapロゴを表示しない。ヘッダー全体とタブ周辺余白をドラッグ領域、実際の操作ボタンやセレクタを `no-drag` 領域にする。通常ブラウザとiOS WebViewにはこの余白やロゴ非表示を適用しない。
...
- デスクトップの `マップ` 画面では、上部中央の旧 `AIで整理` ボタンを置かない。右上操作は `AI実行` の右側に `マップチャット` ボタンを置き、その右にカレンダー/メモ等の右ペイン操作を並べる。`マップチャット` はカレンダー/メモ右ペインと同じ排他枠を使い、押すとマップ右側のリサイズ可能なサイドバーに選択中プロジェクトの `UnifiedChat` を表示する。サイドバー見出しは `マップチャット` とし、会話本文・スターターは通常の `UnifiedChat` と共通にする。マップチャットの空状態では `何をしますか？` の上に選択中プロジェクトのチャット履歴を最大3件だけ縦表示し、各履歴行は1本線の区切りで見せる。`最近1ヶ月` のようなタブは出さず、`もっと表示する` またはヘッダーの履歴アイコンを押すと、同じチャット領域内でプロジェクト履歴一覧へ切り替える。履歴行を押すとそのチャットを開き、`戻る` でスターターへ戻す。マップチャット中の新規チャットは通常チャットではなく、右上ヘッダーのアイコンから選択中プロジェクトの `project:<id>` スコープで作成する。選択プロジェクトを変えた場合は、開いているPCサイドバーのプロジェクトチャットも新しいプロジェクトへ追従する。プロジェクト切替の正は上部の既存プロジェクトセレクタに置き、マップとチャットの対象がズレないようにする。
- `マップチャット` からのマインドマップ変更は、原則として本番DBを直接変更せず、プロジェクト全体の `AI案` 下書きへ保存して自動表示する。デフォルト整理範囲は現在マインドマップ上にあるノードのみとし、毎回「Codex未配置も含めるか」を聞かない。ユーザーが「Codex取り込みチャットも含めて」「未整理メモも含めて」など明示した時だけ、Codex Inbox / メモ / ノート見出しも整理対象に含める。AIが範囲拡張が必要だと判断した場合だけ、提案前に一度確認してよい。
...
- 実装上の正本は、DBが `mindmap_drafts` / `mindmap_draft_nodes` / `mindmap_draft_history`、APIが `/api/mindmap/drafts`、`/api/mindmap/drafts/[draftId]/nodes`、`/api/mindmap/drafts/[draftId]/apply`、`/api/mindmap/draft-history/[historyId]/undo|redo`。AIツールは `saveMindmapDraft` を下書き保存専用に使い、`proposeMindmapOrganization` は読み取り専用にする。永続プロジェクトチャットの実行では `agent_chat_sessions.id` を `saveMindmapDraft` へ自動注入し、下書きの `chat_session_id` に保存することで、確定/Undo/Redo後の短い完了メッセージを同じ `マップチャット` へ戻す。マップUIは `mindmap_drafts` / `mindmap_draft_nodes` のRealtimeと `focusmap:mindmap-draft:changed` イベントでactive draftを再読込し、AI案上の手動追加・移動・タイトル変更は下書きノードAPIへ保存する。確定時に `useUndoRedo` へDB履歴ベースのUndo/Redo actionを登録し、Cmd/Ctrl+Z系の既存導線へ接続する。AI案確定時に新規 `tasks` を作るinsert payloadには、確定に不要なレイアウト列 `node_width` / `mindmap_collapsed` を含めない。ノード幅の永続化自体は `tasks.node_width` migration が適用済みの環境だけで使う。
- `マップチャット` と `AI案` 下書きの実装分割、DB案、モックアップは [docs/plans/archive/20260616-mindmap-map-chat-ai-draft.md](plans/archive/20260616-mindmap-map-chat-ai-draft.md) を参照する。
...
- `codex://` deep link 経由の新規スレッド作成・リポジトリ選択・貼り付け済み送信は、OS/アプリ側の公開API制約により完全自動化できない前提。明示auto導線だけ `ws://127.0.0.1:7878` のCodex app-serverへ送る。Focusmapはユーザー向け状態を `未送信` / `実行中` / `確認待ち` / `完了済み` / `接続失敗` へ丸め、`/api/codex/sync-node` とCodex state DB / rollout JSONL から状態確認とログ同期する。Codex state DB は `FOCUSMAP_CODEX_STATE_DB_PATH` があればそれを最優先し、未指定時は現行Codex Desktopの `~/.codex/sqlite/state_5.sqlite` を優先、存在しない場合だけ旧 `~/.codex/state_5.sqlite` へfallbackする。
- Focusmap Macアプリ同梱のNext/API、`focusmap-agent`、`/api/codex/sync-node`、互換 `scripts/task-runner.ts`、debug toolは全て同じCodex state DB resolverを使う。旧 `~/.codex/state_5.sqlite` を直書きしたbundleや常駐プロセスはstale扱いにし、Codex DesktopのDB配置変更後やMacアプリ更新後はMacアプリ/agent/supervisorを再インストールまたは再起動してから監視結果を見る。既知threadが見つからない場合も、canonical resolverで現行DBを読んでいることを確認する前に削除扱いにしない。ローカルDBで一時的に見つからない場合は `thread_unavailable` / `awaiting_approval` として監視を継続し、再び読めた時に `running` / `awaiting_approval` へ復帰させる。
- Focusmap Lite `scripts/focusmap-agent` は、Codex.appまたはCodex CLIを検出したMacでは `codex_app` executorをheartbeatに含める。`codex_app` taskをclaimして自動送信するのは `dispatch_mode='auto'` の時だけで、その場合は `ws://127.0.0.1:7878` のCodex app-serverへ `initialize` → `thread/start|thread/resume` → `turn/start` を送る。`dispatch_mode='manual'` のtaskは人間がCodex.appで送信した後の監視・同期対象であり、agentが標準で自動turn/startしない。Codex thread monitorは1秒tickを維持し、監視対象task/import scopeの再取得は3秒基準にする。これにより、リポ監視ON後やCodex.app側で作業を開始した直後の取り込み・`実行中` 反映を3秒以内へ寄せる。
...
- モバイルの `取り込み` タブでは、下部トレイのチャットカードを長押しするとカードをリフトし、シートを下端の小さなトレイへ縮めてマップを広く見せる。ドラッグ中はノードに近づくと対象ノードを強調し、dropで既存taskの `project_id` と `parent_task_id` を現在マップへ更新して、そのチャットを対象ノードの子ノードとして配置する。drop後はチャットカードをトレイから消し、マップ上では通常のノード形状として表示する。チャットカードをタップすると該当 `ai_tasks` のprogress詳細drawerをスマホ全画面で開き、`Codexチャット` からCodex側threadへ移動できる。`配置先を選ぶ` で次にタップしたノードへ配置する時も、保存APIやRealtime反映を待たず、ローカルの仮配置で対象チャットを即座にマップ上へ表示し、紐づく `ai_tasks` / task-progress snapshot の `実行中` / `確認待ち` バッジと実行中orbitを同じフレームで出す。保存失敗時だけ仮配置を戻し、取り込みカードを復帰させる。titleはCodex.app sidebar/SQLite `threads.title` に入るCodex生成要約を正にし、初回取り込み時にtitleが未生成、raw prompt風、または `first_user_message` の長い先頭部分なら新規カードを作らず、後続tickで生成titleを確認するまでスキップする。過去仕様で本文1行目由来の仮見出しになった既存カードだけは、後続monitorが `threads.title` の生成要約を取得できた時に、未編集タイトルとして `tasks.title` とmemo先頭のMarkdown見出しを後追い更新する。markdown本文は `tasks.memo` に保持し、取り込み一覧カードの本文には `## 初回依頼` のプロンプト本文を表示し、ノード上には短い本文プレビューとCodex状態バッジだけを出す。新規にCodex threadを取り込む時は、`tasks.memo` に `Thread ID`、`Repository`、最終更新、初回依頼、最新プレビューをMarkdownで保存し、`tasks.codex_thread_id`、`tasks.codex_work_dir`、紐づく `ai_tasks.result.codex_thread_url` / `codex_run_state` も保持する。長押しD&Dに失敗する端末やアクセシビリティ用に、カード詳細から `配置先を選ぶ` を開き、次にタップしたノードへ配置する代替導線も用意する。
...
- マインドマップ右上のCodex系操作は、Web側のproject設定と既存DB状態を更新・表示するためのもの。常駐runnerの即時スキャン強制やCodex内部ファイルの直接読み取りは行わない。
...
- 実行中はMac側で Codex state DB / rollout JSONL / app-server通知を1秒基準で確認してよい。`focusmap-agent` のCodex monitor loopは通常1秒tickだが、監視対象taskと取り込みscopeのAPI再取得も3秒基準にし、scope取得後の直接開始thread検出は同じ1秒tick内でローカルsqliteを読む。看板・一覧向けのWeb UIは3秒pollを基準にし、Turso snapshotは `runner_heartbeats.current_task_id`、`last_seen_at`、`ai_tasks.last_activity_at/current_step/summary` の短い状態だけを読む。Mac `focusmap-agent` の通常flushは2秒最短だが、hashまたは活動時刻が変わった時と状態変化時だけにし、同一hashや同じ活動時刻なら送信しない。thread検出・running再開・確認待ち・完了・失敗など状態変化だけforce送信する。これにより、リポ監視ON直後やagent再起動直後の新規チャット反映は3秒scope refresh + UI 3秒pollを基準にし、scope取得済みならローカル検出はおおむね次の1秒tickで走り、DB書き込みは変化時だけに絞る。
- メモからCodexへ渡す導線と活動表示の詳細計画は `docs/specs/memo-codex-execution/requirements.md` / `delivery-plan.md` を参考にする。メモ詳細・マップノード詳細・リンクメモ詳細の初回Codex操作は、Focusmap Macアプリ内でも通常ブラウザ、ローカルWeb、Cloudflare preview、スマホでも `executor='codex_app'` / `dispatch_mode='manual'` の handoff task を作り、プロンプトのクリップボードコピーと Codex.app / ChatGPT Codex 入口の起動だけを補助する。Codex.appへ最終送信するのは人間。Macアプリ内であることを理由に `dispatch_mode='auto'` へ切り替えない。作成から10分以内で `codex_thread_id` が未保存のmanual handoffは、`focusmap-agent` が Codex state DB の `first_user_message` をhandoff tokenまたはprompt先頭で照合し、初回thread検出をMac側巡回だけで保存する。確認待ち後にCodex側で追加プロンプトが送られた場合は、`focusmap-agent` の固定thread monitorが `awaiting_approval_at` / `last_activity_at` より後の `user_message` または `task_started` だけを見て再開を検知し、通常でも1秒以内を目安に `running` snapshot/eventへ戻す。thread `updated_at_ms` 単体は再開根拠にしない。
...
- Codex.app側で連携中threadをアーカイブした場合は、人間がそのCodex作業を片付けた合図として扱う。通常の `focusmap-agent` monitorは次の同期で対象 `ai_tasks.status='completed'` / `completed_at` を保存し、元ノードをチェック済みにする。理由は `ai_tasks.result.codex_review_reason='archived'` に残す。sqliteから一時的に読めなくなったthreadは `thread_unavailable` の確認待ちとして扱い、監視を継続し、元ノードを勝手に完了しない。通常のCodex実行完了 `completed` や承認待ちでも元ノードを勝手に完了しない。逆方向では、Focusmapのノードチェック直後は `ai_tasks.result.codex_archive_request_state='waiting_for_grace'` に留め、同じ端末で10秒以上チェックが維持された時だけ `codex_archive_request_state='pending'` / `codex_archive_requested_at` を保存する。マインドマップ上のCodex由来ノードを削除する時も、削除操作そのものを片付け意思として扱い、猶予なしで `codex_archive_request_reason='node_deleted'` のpending requestを作る。スマホやWebはMac内のCodexを直接操作せず、Mac側 `focusmap-agent` が `/api/agents/codex-monitor/tasks` からpending requestを拾い、ローカル `codex app-server` へ `thread/archive` を送る。チェック解除時はタイマーを止め、保存済みrequestも `cancelled` として扱うため、Mac agentはアーカイブしない。
- 確認待ち・手動貼り付け待ち・needs_inputは、Mac側ではCodex.app sqlite/rollout/app-serverを1秒単位で確認する。ただしDBへは無変化の `codex_last_checked_at` だけを書かず、人間がCodex.appへ追加入力して `running` に戻った時、thread検出、確認待ち化、thread一時未検出、thread archiveによる完了、失敗などの状態変化だけをUI詳細表示中は約3秒、通常の `focusmap-agent` monitorでは1秒以内を目安にsnapshot/eventへ反映する。
- `/api/agents/codex-monitor/tasks` は、通常のactive Codex taskに加えて、`completed` でも `codex_archive_request_state='pending'` かつ元ノードがまだ `done`、または削除操作で元ノードが既に無いtaskだけをMac agentへ返す。これによりスマホからチェックした完了ノードでもMac agentがCodex threadを閉じに行ける一方、10秒以内の誤チェック解除やキャンセル済みrequestはMacへ渡らない。互換/debug用の旧 `scripts/task-runner.ts` も同じpending requestだけをarchive対象にし、単に元ノードがdoneになっただけでは即アーカイブしない。
...
- スマホのCodexシートは `取り込み` と `看板` の2タブに分ける。`取り込み` は選択中projectの `Codex Inbox` 配下にある `source='codex_app_thread'` ノードのうち、repo pathが一致し、かつ `未送信` / thread未検出ではないものだけをカード表示する。カードにはCodex thread title由来のタイトル、`tasks.memo` の `## 初回依頼` から抽出したプロンプト本文、repo名、状態、最終更新を出し、thread IDは表示しない。チャット詳細はカード全体のタップで開き、カード内に大きな `チャットを見る` ボタンは置かない。`配置先を選ぶ` でシートを閉じてマップ上の配置モードへ入る。配置モード中はノードタップで編集を開かず、選んだノードの子として取り込みノードを移動し、親ノードが折りたたまれていれば展開する。
...
- Codex.appでFocusmapを経由せず直接開始したthreadは、Macの `focusmap-agent` が Codex state DB（既定は `~/.codex/sqlite/state_5.sqlite`、旧配置はfallback）を読む。これはWeb pushではなく、Macローカルagent/Electron supervisorとFocusmap APIの協調機能である。取り込み対象は `/api/agents/codex-monitor/import-scopes` が返す `projects.repo_path` と完全一致する `threads.cwd` だけで、かつ `projects.codex_thread_import_enabled=true`、`codex_thread_import_enabled_since` 以降に更新された非archived threadに限る。project側のON/OFFはデスクトップではマインドマップ左上のCodexボタン、スマホでは右上Botから開くCodexシート内の `リポ監視` で行い、ON時に開始時刻を保存する。repo未設定projectではボタン/スイッチを無効化し、API側もONを拒否する。`repo_path` はブランチ名やremoteではなくローカルGitリポジトリのフォルダpathだけを保存し、Focusmap agentの `available_repos` に未掲載でもFinder/手入力の絶対pathがGit管理下なら `git rev-parse --show-toplevel` のrootへ正規化して保存できる。Finder選択はMacアプリ内ではElectron IPC `window.focusmapDesktop.chooseFolder()` でローカルFinderを開いて即保存し、通常の本番ブラウザではCloud Run上の `/api/codex/choose-folder` を呼ばず候補/手入力へフォールバックする。`repo_path` を変更/解除した場合は取り込みをOFFに戻し、変更後のrepoはCodex取り込みONにした時刻以降だけ取り込む。agentは既存manual handoff taskの同期を先に維持し、直接開始thread取り込みだけをこのscope内で追加実行する。`source_task_id` を持つFocusmap manual handoff taskと同じ `cwd` / 初回promptに一致するthreadは、既にマップノードへ紐づく実行として扱い、リポ監視の未配置チャット候補には取り込まない。SQLite検索もscope repoの `cwd` に絞り、scope外repoの直近threadに埋もれて対象repoの未取り込みthreadへ届かない状態を避ける。候補判定は `first_user_message` を正にし、現行Codex Desktopで実prompt行でも `has_user_event=0` になるケースがあるため `has_user_event` には依存しない。
...
- `Codex Inbox` は直接開始thread取り込みの内部受け皿であり、デスクトップ/スマホのマインドマップ上には自動表示しない。マップ描画時は `source='codex_inbox'` または `title='Codex Inbox'` のノードとその配下の未配置threadを除外し、チャット取り込みサイドバー/スマホCodexシートからドラッグ&ドロップまたは「配置先を選ぶ」で通常ノードへ移したものだけマップに表示する。デスクトップでチャット履歴カードをマップへドラッグしている間は、ポインタに最も近いproject/taskノードを配置候補として強調し、taskノード中央は `子ノードにする`、上端は `上に並べる`、下端は `下に並べる` として扱う。子ノード配置では点線コネクタ、`ここに紐づく` バッジ、ドラッグ中チャット名のゴーストノードを表示し、兄弟配置では対象ノード上/下の挿入線を表示する。drop時は `as-child` なら対象ノードを親にし、`above` / `below` なら対象ノードと同じ `parent_task_id` に移して同じ親配下の `order_index` を並べ直す。候補が無い空白dropだけ `project-root` 直下へ入れる。配置済みの `source='codex_app_thread'` ノードはチャット履歴詳細フッターの `履歴へ戻す` で、同じリポの `Codex Inbox`（現在projectのInboxを優先、無ければ同repoのInboxへfallback）配下へ `parent_task_id` / `project_id` を戻し、マップから再び隠して未配置履歴に復帰させる。スマホCodexシートの `取り込み` タブはデスクトップ同様にリポ単位で表示し、`available_repos`、projectの `repo_path`、取り込み済みthreadの `codex_work_dir` からリポ候補を作り、リポ選択/選択解除/`リポ監視` ON/OFFを行える。未送信/prompt waiting のものはデスクトップ/スマホとも取り込み一覧に出さない。
