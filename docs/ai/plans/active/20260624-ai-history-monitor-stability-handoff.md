...
| `scripts/task-runner.ts` | 旧ローカルrunner。過去のCodex監視・task実行互換経路を持つruntime | `focusmap-agent` と二重writerになると重い/危険 | 今回の主作業には混ぜない。通常Codex監視はOFF、legacy/debugだけにする方針を維持 |
...
|---|---|---|---|
| `scripts/task-runner.ts` 全体退役 | 旧runnerのclaim、Codex互換監視、legacy実行 | 別の一本化計画の範囲。今回に混ぜるとAgent hot path修正がぼやける | `20260607-codex-mac-agent-unification.md` 側で完了判断する |
| 旧 `task-runner` Codex監視 | 旧runnerがCodex SQLite/rolloutを見るdebug互換経路 | 通常OFF方針は既にある。今回の実装対象は `focusmap-agent` | 旧監視が通常起動しないことをIntegrationで再確認 |
