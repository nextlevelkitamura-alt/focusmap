...
- `/api/ai/agent` は Codex 風のツール実行エージェントとして扱う。通常会話だけで終わらせず、必要に応じて `listProjects` / `getProjectContext` / `saveProjectContext` / `listProjectTasks` で Focusmap DB のプロジェクト概要・蓄積コンテキスト・タスクを確認/記録し、`listCalendarEvents` / `checkCalendarAvailability` / `addCalendarEvent` / `updateCalendarEvent` で既存予定の確認、空き時間確認、新規予定作成、見出し・内容・時間変更を行う。Mac が online の時は既存の `runTerminal` / `listFiles` / `readFile` / `writeFile` / `runOpenCode` / browser 系ツールも使える。低リスクな確認・作成・更新は自律実行するが、削除、大量変更、外部公開、送信、課金、権限変更、秘密情報の表示や保存は事前確認し、Mac agent 側の危険コマンドブロックと許可ルートを迂回しない。通常チャットで対象プロジェクトや対象予定が曖昧な時は、検索ツールで候補を出してから確認する。
