...
- メモ詳細の見出し/本文は、入力中のローカルdraftを端末側の正として扱い、DB保存完了前に古いDB値で入力内容を巻き戻さない。`WishlistCardDetail` はメモIDごとに `localStorage` へ未同期draftを保存し、同じメモを開き直した時にDB値と異なるdraftを復元する。PCは従来通り短いdebounceで自動保存するが、表示は `端末に保存済み` / `同期中` / `保存済み` / `未同期・再試行` に分け、DB ACK前に保存済み扱いにしない。スマホは強い緑の `保存` を押した時だけDBへ反映し、保存失敗時はシートを閉じず、端末draftを保持して近くにエラーを表示する。
