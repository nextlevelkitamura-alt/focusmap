...
/**
 * モデル選定:
 *   - DEEPSEEK_API_KEY 設定時: DEEPSEEK_INTENT_MODEL (デフォルト deepseek-v4-flash) を使う
 *     intent判定は軽量タスクなので Flash が最適、 Pro は agent tier (focusmap-agent側) で使用
 *   - 未設定時: Gemini Flash-Lite にフォールバック
 *
 * 注: deepseek-chat / deepseek-reasoner は 2026-07-24 廃止予定
 *     → deepseek-v4-flash (非Thinking) / deepseek-v4-pro (推論重視) に移行
 */
function getModel() {
  if (process.env.DEEPSEEK_API_KEY) {
    const modelId = process.env.DEEPSEEK_INTENT_MODEL ?? process.env.DEEPSEEK_MODEL ?? 'deepseek-v4-flash';
...
export function getActiveModelLabel(): string {
  if (process.env.DEEPSEEK_API_KEY) {
    return process.env.DEEPSEEK_INTENT_MODEL ?? process.env.DEEPSEEK_MODEL ?? 'deepseek-v4-flash';
  }
