...
- デスクトップ通常 `メモ` 画面の看板ボードは `未予定` / `今日` / `完了` の3列に絞る。左の追加パネルは常に見出し・本文・画像・保存ボタンを表示し、×はパネルを隠さず入力内容のクリアだけにする。`未予定` は横幅を広く取り、カード自体を長くするのではなくカードを最大3列グリッドで多く表示する。`今日` と `完了` も狭くしすぎず、カード本文が読める最低幅を確保する。予定済み・マップ追加済みのDB状態は壊さず、今日/完了でないアクティブメモはデスクトップ看板上では `未予定` に集約し、カード内の既存チップで予定済み/マップ追加済みを示す。`未予定` と `今日` の見出し右側には小さな＋を置き、押すと左の追加パネルの追加先をその列に合わせる。`完了` には追加＋を置かない。メモカードは左上にプロジェクト、見出し下に重複排除したタグ、右上に完了チェックを置き、今日するアイコンと一覧カード右側のCodex/クリップボード導線は表示しない。スマホ看板とコンパクト/分割/リンクメモ導線は従来の看板・入力仕様を維持し、スマホ看板ではカードを左右端へ短く保持すると隣の看板へスクロールし、ドロップ時にカラム移動として保存する。タグ絞り込みとメモ選択マップ化の処理はコード上に残すが、通常UIの上部ツールバーからは非表示にする。タグ候補は編集シート内でスマホでも押しやすいチップサイズを維持する。
