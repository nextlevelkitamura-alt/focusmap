...
- デスクトップの `マップ` 画面では、上部中央の旧 `AIで整理` ボタンを置かない。右上操作は `AI実行` の右側に `マップチャット` ボタンを置き、その右にカレンダー/メモ等の右ペイン操作を並べる。`マップチャット` はカレンダー/メモ右ペインと同じ排他枠を使い、押すとマップ右側のリサイズ可能なサイドバーに選択中プロジェクトの `UnifiedChat` を表示する。サイドバー見出しは `マップチャット` とし、会話本文・スターターは通常の `UnifiedChat` と共通にする。マップチャット内の履歴/詳細サイドバーは初期表示せず、ユーザーが履歴ボタンを押した時だけ開く。マップチャット中の新規チャットは通常チャットではなく、選択中プロジェクトの `project:<id>` スコープで作成する。選択プロジェクトを変えた場合は、開いているPCサイドバーのプロジェクトチャットも新しいプロジェクトへ追従する。履歴を開いた時の表示形式は別途決めるが、プロジェクト切替の正は上部の既存プロジェクトセレクタに置き、マップとチャットの対象がズレないようにする。
