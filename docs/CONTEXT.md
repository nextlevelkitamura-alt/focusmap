...
- マインドマップのメモ編集パネル（`CodexNodePanel`）では、「Codexに送る」から同じ手動ハンドオフを実行する。ユーザー本文だけのpromptで `ai_tasks` を作り、プロンプト本文はdeep link queryへ載せない。Mac/デスクトップではtracking task作成後にpromptなしの `codex://?path=...&originUrl=...` またはローカルAPIでCodex.appを開く。スマホではOSの外部アプリ起動制約を優先し、タップ直後に `dispatch_mode='manual'` の `ai_tasks` 登録リクエストと同期クリップボードコピーを先に開始してからChatGPT Codex入口を起動する。Focusmapが `visibilitychange` / `pagehide` / `blur` / 復帰を検知した時、またはFocusmap iOSアプリWebViewが `focusmap:openExternal` bridgeを送った直後に `/api/ai-tasks/[id]/manual-handoff` で `prompt_waiting` を `awaiting_approval` へ進め、Supabase/Turso/activityへ「外部アプリ確認待ち」を保存する。スマホUIの主ボタン名は開始前・未送信詳細とも `Codexを開く` にする。通常ブラウザ/CloudflareプレビューではiOSは公式Codex mobile URL `https://chatgpt.com/codex/mobile/`、Androidは `intent://chatgpt.com/codex/mobile/#Intent;scheme=https;package=com.openai.chatgpt;S.browser_fallback_url=https%3A%2F%2Fchatgpt.com%2Fcodex%2Fmobile%2F;end` を使い、OS側にChatGPTアプリ起動を試させる。Focusmap iOSアプリ内WebViewではpromptがあれば `focusmap:copyText` を送り、その後 `focusmap:openExternal` でReact Native側へ渡す。iOSアプリには `urls` 候補として公式URL → `chatgpt://codex/mobile` → `chatgpt://codex` → `com.openai.chat://codex/mobile` → 汎用 `chatgpt://` / `com.openai.chat://` を渡す。再インストール済みアプリでは公式URLを `UIApplication.open(..., universalLinksOnly: true)` で先に試し、ChatGPTアプリがCodex URLとして扱える場合だけ成功させる。汎用schemeはChatGPTアプリを開くだけでCodex画面直行を保証しないためfallback扱いにする。
...
- localhost、`*.local`、`*.trycloudflare.com` のローカル/スマホプレビューでは、デスクトップの場合だけローカルAPI `/api/codex/open-repo` からMacの `pbcopy`、`open codex://...`、Codex.appのactivateを実行する。スマホ判定時はMac側のローカルAPIへ送らず、端末側クリップボードへの同期コピーとChatGPT mobile入口を優先する。ブラウザまたはFocusmap iOSアプリが外部アプリへ画面を切り替えたら、ChatGPT側の会話本文はFocusmapから直接読めないため、ローカル判定で `awaiting_approval` に進める。外部アプリ起動自体が止まり画面切替も発生しない場合だけ `prompt_waiting` / `未送信` として再コピー・再起動導線を残す。
...
- `Codexに送る` 直後のmanual handoffは `prompt_waiting` / `未送信` として表示する。Mac/デスクトップのCodex.app threadは、Codex側で送信・活動が確認できるまでは `未送信` のままにする。スマホChatGPTアプリ handoff は thread/rolloutを読めないため、Focusmapの画面切替または復帰を検知した時点で `codex_run_state=awaiting_approval` / `codex_review_reason=external_app_handoff` に進める。threadの `updated_at_ms` や `last_activity_at` が直近の3秒UI pollで動いていれば `実行中`、Mac heartbeatはあるがCodex activityが止まっていれば停止疑いまたは `確認待ち` とする。ユーザー可視の出力本文やpreviewは通常snapshotへ保存しない。
