...
- モバイル `チャット` は全画面の会話面を正にし、左端スワイプまたは左端の取っ手でチャット履歴シートを開く。履歴シートは `チャット`、`プロジェクト`、`最近` の順で構成し、`ライブラリ` は置かない。`プロジェクト` はヘッダーで選んだスペース内のプロジェクトを表示し、タップするとそのプロジェクト専用チャットへ切り替わる。履歴は現在のチャットスコープ内で更新日時の新しい順に扱う。通常表示は最近3件だけを見せ、`もっと表示する` / `表示を減らす` で履歴の展開状態を切り替える。通常チャット画面でも入力欄の上、右下に新規チャットFABを出し、下部ナビ・safe area・入力欄と重ならない位置へ置く。
