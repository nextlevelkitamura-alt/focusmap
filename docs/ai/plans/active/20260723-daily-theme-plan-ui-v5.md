...
- `ThemeGroupCard` を複数Themeのアコーディオンへ変更し、初期状態はすべて閉じる。閉じたカードの右側には進捗率・工程・進捗バー・計画数・active/planning・稼働/確認待ちを集約する。鉛筆編集は別パネルを出さず、カード内のテーマ名・目的をそのまま入力枠に置き換え、完了条件は目的の直下へ出す。
