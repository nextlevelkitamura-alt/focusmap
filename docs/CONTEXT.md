...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・タスク化・調査などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。入力欄に固定の `実行` ピルは出さず、追加メニュー・音声・送信だけに絞る。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。デスクトップではチャット専用の2段目ヘッダーを置かず、既存のダッシュボード上部バーへ履歴開閉と新規チャットを統合し、チャットを開いた時は履歴サイドバーを初期表示する。ヘッダーではスペース切替だけを出してプロジェクト切替は出さない。Web版の左上はFocusmapロゴ/ホーム導線とチャット操作、Macデスクトップ版の左上はネイティブ信号ボタン余白の右にチャット操作とスペース切替だけを置き、同じ場所に重複したブランドやチャットタイトルを出さない。
...
- `/api/ai/agent` は Codex 風のツール実行エージェントとして扱う。通常会話だけで終わらせず、必要に応じて `listProjects` / `getProjectContext` / `saveProjectContext` / `updateProject` / `listProjectTasks` で Focusmap DB のプロジェクト概要・蓄積コンテキスト・タスクを確認/記録/更新し、`getMindmapOverview` / `getMindmapNodeDetail` / `updateMindmapNode` / `moveMindmapNode` / `updateMindmapMemoLink` でマインドマップDBのノード一覧、親子関係、進捗、予定情報、メモ紐づき、ノード移動、メモ紐づき付け替えを扱う。`listCalendarEvents` / `checkCalendarAvailability` / `addCalendarEvent` / `updateCalendarEvent` で既存予定の確認、空き時間確認、新規予定作成、見出し・内容・時間変更を行う。Mac が online の時は既存の `runTerminal` / `listFiles` / `readFile` / `writeFile` / `runOpenCode` / browser 系ツールも使える。低リスクな確認・作成・更新は自律実行するが、削除、大量変更、外部公開、送信、課金、権限変更、秘密情報の表示や保存は事前確認し、Mac agent 側の危険コマンドブロックと許可ルートを迂回しない。通常チャットで対象プロジェクト、対象ノード、対象予定が曖昧な時は、検索ツールで候補を出してから確認する。
