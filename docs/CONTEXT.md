...
- `マインドマップを整理` / `マップ整理` の入口では、整理案を出す前に必ず「Codexから未配置/未取り込みのチャット・作業も含めるか」「現在マインドマップ上にあるノードだけで整理するか」を聞く。範囲が決まった後に、AIは新規ノードを作って紐づける案と、既存ノード配下へ入れる案の両方を検討して提案する。
- マップ上の `AIで整理` 導線は、スマホでは従来通り `チャット` 全画面のプロジェクトチャットを開く。デスクトップでは画面遷移せず、マップ右側のリサイズ可能なサイドバーに `UnifiedChat` のプロジェクトチャットを表示する。PCサイドバーはカレンダー右ペインと同じ排他枠を使い、開いた時はカレンダー/メモ分割を閉じる。上部には `AIで整理`、対象プロジェクト名、履歴開閉、新規チャット、閉じるを置き、会話本文とスターターは通常の `UnifiedChat` と共通にする。選択プロジェクトを変えた場合は、開いているPCサイドバーのプロジェクトチャットも新しいプロジェクトへ追従する。
...
- `/api/ai/agent` は Codex 風のツール実行エージェントとして扱う。通常会話だけで終わらせず、必要に応じて `listProjects` / `getProjectContext` / `saveProjectContext` / `updateProject` / `listProjectTasks` / `listNotesForOrganization` / `getNoteOrganizationDetail` で Focusmap DB のプロジェクト概要・蓄積コンテキスト・タスク・ノート/メモを確認/記録/更新する。プロジェクトを読んだ後は、読んだ概要・現状・タスク/マップの要点を短く共有し、この前提で何を整理するかを聞く。プロジェクトチャットの初期文脈では `tasks` からノード件数・見出し件数・主な見出し、`ideal_goals` / `memo_items` からノート/メモ件数・主な見出しだけを読む。ノート/メモ整理では `listNotesForOrganization` が見出しと詳細冒頭30文字を標準で返し、AIは本文全量を無差別に読まず、判断に必要な候補だけ `getNoteOrganizationDetail` で詳細文字数を増やす。マインドマップ整理相談では、整理対象の範囲確認後に読み取り専用の `proposeMindmapOrganization` でプロジェクト概要・蓄積コンテキスト・見出しレベルのツリー・必要に応じたノート見出し・まとめ候補・図解ハーネスを取得し、DB変更前に「新規ノード作成案」「既存ノードへ入れる案」「移動対象」「理由」「実行操作」を `text` ツリー図で提案して承認を取る。承認後だけ `addMindmapGroup` でまとめノードを作り、返却された `group.id` を親にして `moveMindmapNode` で既存ノードを移す。マインドマップの深掘りは、必要になった時だけ `getMindmapOverview` / `getMindmapNodeDetail` / `updateMindmapNode` / `moveMindmapNode` / `updateMindmapMemoLink` でノード一覧、親子関係、進捗、予定情報、メモ紐づき、ノード移動、メモ紐づき付け替えを扱う。プロジェクト状況を保存する時は、`projects.description` を安定した概要、`project_contexts.details` をAGENTS.md風の読みやすい背景メモ、`project_contexts.progress` を現在地・次の論点・ブロッカーの状況メモとして使う。`details` は必要に応じて `## 目的` / `## 判断基準` / `## 重要制約` / `## 最近の決定`、`progress` は `## 現在地` / `## 次の論点` / `## ブロッカー` のような小見出しで整理し、マインドマップ整理の判断材料にする。`listCalendarEvents` / `findCalendarOpenSlots` / `checkCalendarAvailability` / `addCalendarEvent` / `updateCalendarEvent` / `deleteCalendarEvent` で既存予定の確認、複数日の空き枠検索、新規予定作成、見出し・内容・時間変更、予定削除を行う。予定削除は `listCalendarEvents` で `google_event_id` と `calendar_id` を確認し、ユーザーの削除意思と対象が一意に明確な場合だけ実行する。Mac が online の時は既存の `runTerminal` / `listFiles` / `readFile` / `writeFile` / `runOpenCode` / browser 系ツールも使える。低リスクな確認・作成・更新は自律実行するが、削除、大量変更、外部公開、送信、課金、権限変更、秘密情報の表示や保存は事前確認し、Mac agent 側の危険コマンドブロックと許可ルートを迂回しない。通常チャットで対象プロジェクト、対象ノード、対象メモ、対象予定が曖昧な時は、検索ツールで候補を出してから確認する。
- マインドマップ整理相談で `proposeMindmapOrganization` を呼ぶ時は、範囲確認済みの回答に合わせて `includeCodexInbox` と `includeNoteHeadings` を渡す。未指定/falseの `includeCodexInbox` では `Codex Inbox` 配下の未配置/未取り込み扱いのCodexチャット・作業を除外し、trueではそれらも整理対象に含める。「マップ上だけ」の回答では `includeNoteHeadings=false` とし、メモ/ノートも含めたいとユーザーが明示した場合だけ `includeNoteHeadings=true` にする。DB変更前の提案には「新規ノード作成案」「既存ノードへ入れる案」「移動対象」「理由」「実行操作」を含める。
