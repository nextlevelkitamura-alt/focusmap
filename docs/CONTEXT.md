...
- デスクトップマップ右上のCodexロゴボタンは `チャット取り込み` 右サイドバーを開く。サイドバーはproject単位ではなく選択中のリポ単位の未配置Inboxとして扱い、Mac側 `focusmap-agent` が登録した `available_repos` 候補またはFinder選択で対象リポを切り替える。通常UIには長いパス手入力欄を出さず、候補名とFinder導線を主にする。`リポ監視` は選択リポに対して有効な `projects.codex_thread_import_enabled` scopeがあるかを表示し、ONにする時は現在projectの `projects.repo_path` を対象リポへ合わせてscopeを作る。OFFにする時は同じリポの有効scopeを止める。サイドバー下部は選択リポの `source='codex_app_thread'` かつ `codex_work_dir` が一致し、どこかの `Codex Inbox` グループ直下に残っている未配置チャットだけを表示する。現在project外に取り込まれた同一リポのチャットも一覧に出し、チャット行を現在マップのノードまたはproject rootへドラッグすると、既存taskの `project_id` と `parent_task_id` を現在マップへ更新して配置する。配置後は未配置一覧から消える。チャット行の削除は既存taskを削除し、グローバルUndoで削除前task snapshotを復元できる。UIはCodex sqlite/rolloutを直接読まず、取り込み済みノードとtask-progress snapshotの読み取りだけで一覧を作る。配置済みチャットは複製しない。
