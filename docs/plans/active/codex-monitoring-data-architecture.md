...
- UI上のCodex状態は `未送信` / `実行中` / `確認待ち` / `完了済み` / `接続失敗` に丸める。
- `pending` は `未送信`、`running` は `実行中`、`awaiting_approval` / `needs_input` / `completed` は `確認待ち`、元ノードがチェック済みのsnapshotだけ `完了済み`、`failed` は `接続失敗` と表示する。
- manual handoffの `pending` でも、Codex.app threadの出力previewやmessage/current_stepが取れたら `未送信` から外す。thread更新時刻やlast_activityが直近45秒以内なら `実行中`、出力が止まっていれば `確認待ち` とする。thread検出だけで出力が無い場合は `未送信` のままにする。
- Codexが停止しただけではノードを自動完了扱いにしない。人間が見てチェックボックスを入れた時、またはCodex threadのアーカイブでsource nodeを閉じた時だけタスク完了にする。thread missing/deleteは確認待ちに留める。チェックを外した時は紐づくCodex taskを `awaiting_approval` に戻す。
- 看板レーンは `未送信` / `実行中` / `確認待ち` / `接続失敗` / `完了済み`。デスクトップは初期状態を畳んで件数だけを見せ、展開時もマップを圧迫しすぎない高さに抑える。`未送信` はCodex.appでまだ開始されていないもの、`確認待ち` はCodex出力や完了を人間が確認するものに限定し、同じカード内で両方の意味を併記しない。`完了済み` はノードがチェック済みで、紐づくprogressの `updated_at` が当日のものだけを一時表示する。翌日以降は看板から消す。
...
- running中・detail open中・runner heartbeatはいずれも5秒を基準にする。active watchは「詳細を見ているtask」をMacへ知らせるが、3秒boostはしない。
...
- ノード詳細のCodex同期は、開いているtaskだけ `/api/codex/sync-node` を約5秒で呼び、未送信から実行中/確認待ちへの変化を拾う。Supabase/Turso writeは状態変化、thread検出、thread `updated_at_ms` / `last_activity_at` 変化、current_step変化、一定間隔の `codex_last_checked_at` 更新時だけ行い、無変化の5秒pollをDB writeにしない。
- Mac online/offline は `/api/task-progress/runner-heartbeats?limit=5` を5秒pollし、20秒以内のheartbeatをonlineとして表示する。heartbeatは `current_task_id` を含め、看板カードはrunning taskに `5秒pulse` / `Codex停止疑い` / `pulse待ち` を表示する。
...
| 用途 | 推奨頻度 | クラウド送信 |
|---|---:|---|
| Codex実行状態 | 5秒 | 本文なしのpulse JSON |
| runner heartbeat active | 5秒 | current_task_id付きupsert |
| runner heartbeat idle | 5秒 | liveness upsert |
...
| 実行中タスク詳細 | REST polling または SSE | 5秒 |
| 承認待ち/失敗/完了 | APNs push | イベント時 |
| スクショ表示 | metadata取得後に署名付きURL | 表示時だけ |
| バックグラウンド | APNs中心 | 常時pollしない |

初期MVPではSSE/WebSocketを必須にしない。画面を開いている時だけ5秒pollingで十分とする。
...
- [x] `running` だけ短周期更新する。
  - `running` は5秒、`pending` は30秒、確認待ち/完了/失敗/手動貼り付け待ちは低頻度または手動更新にする。
...
- [x] heartbeatを軽量5秒pulseへ変更する。
  - `focusmap-agent` heartbeatは5秒、runner状態UIも5秒polling、オンライン判定窓は20秒にする。本文ログではなく `current_task_id` とagent状態だけを送る。
...
- 一覧/マップ表示: `GET /api/task-progress/snapshot?updated_after=<cursor>&limit=500`
  ...
  - AI画面表示中とマップにrunningノードがある時は5秒poll、runningが無い時は45〜60秒または手動更新。
- 詳細表示: `GET /api/task-progress?task_id=<id>&limit=50`
  - 返却: `{ source, task, progress, events }`
  - 詳細を開いた時だけtailを読む。マップ一覧では読まない。
- Mac agent送信:
  - Codex.app通知はローカルで短周期に拾う。
  - Turso通常progress送信は内容hashまたは活動時刻が変わった時だけ、最短5秒間隔。
  - `codex_thread_ready` / `codex_prompt_sent` / `awaiting_approval` / `completed` / `failed` は即送信。
  - `result.live_log` / `output` / 生のCodex通知全文は送信しない。`current_step` は状態語を600文字以内、`summary` は最終活動時刻つきの稼働シグナルを1200文字以内、`progress_json` は `executor` / `codex_run_state` / `codex_thread_id` / `last_activity_at` / 直近8steps / 文字数などのcompact metadataだけにする。
...
- [x] `running` 以外のCodex状態で短周期監視しない。
- [x] 一覧取得で巨大JSONを取らない。
- [x] runner heartbeatは本文なし5秒pulseに統一されている。
- [x] 実行中タスク詳細は5秒で体感よく更新される。
...
- iPhoneで本当に必要なリアルタイム度は、実行中画面だけ5秒で足りるか。
