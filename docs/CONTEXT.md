...
- デスクトップ通常 `メモ` 画面の看板ボードは `メモ` / `今日` / `完了` の3列に絞る。`メモ` は横幅を広く取り、左パネル表示中はカードを2列、左パネルを閉じた時はカードを最大3列グリッドで表示する。`今日` と `完了` も狭くしすぎず、カード本文が読める最低幅を確保する。予定済み・マップ追加済みのDB状態は壊さず、今日/完了でないアクティブメモはデスクトップ看板上では `メモ` に集約し、カード内の既存チップで予定済み/マップ追加済みを示す。`メモ` と `今日` の見出し右側には小さな＋を置き、押すと左の追加パネルの追加先をその列に合わせる。`完了` には追加＋を置かない。メモカードは左上にプロジェクト、見出し下に重複排除したタグ、右上に完了チェックを置き、今日するアイコンと一覧カード右側のCodex/クリップボード導線は表示しない。スマホ看板とコンパクト/分割/リンクメモ導線は従来の看板・入力仕様を維持し、スマホ看板ではカードを左右端へ短く保持すると隣の看板へスクロールし、ドロップ時にカラム移動として保存する。タグ絞り込みとメモ選択マップ化の処理はコード上に残すが、通常UIの上部ツールバーからは非表示にする。タグ候補は編集シート内でスマホでも押しやすいチップサイズを維持する。
