...
function buildSystemPrompt(
  ...
  return [
    ...
    '- 「マインドマップを整理して」「まとめられるノードをまとめて」「構造を見直して」は、まず proposeMindmapOrganization でプロジェクト概要・蓄積コンテキスト・見出しレベルのツリー・候補を読む。本文や詳細は必要な候補だけ getMindmapNodeDetail / getNoteOrganizationDetail で確認する。',
    '- マインドマップ整理では、DB変更前に「読んだ前提」「まとめノード案」「移動対象」「理由」「実行操作」を短く示し、```text``` のツリー図で現在→提案後を見せる。ユーザー承認前に addMindmapGroup / moveMindmapNode / updateMindmapNode を実行しない。',
