...
- `pending` は `未送信`、`running` は `実行中`、`awaiting_approval` / `needs_input` は `確認待ち`、`completed` は `完了済み`、`failed` は `接続失敗` と表示する。
...
- Codexが停止しただけではノードを自動完了扱いにしない。人間が見てチェックボックスを入れた時、またはCodex threadのアーカイブ/削除でsource nodeを閉じた時だけタスク完了にする。チェックを外した時は紐づくCodex taskを `awaiting_approval` に戻す。
