...
- 自作マップのノード折りたたみ状態は `tasks.mindmap_collapsed` を正にする。マップを開いた時は `groups` / `tasks` のDB値から折りたたみIDを復元し、折りたたみ/展開ボタン、子追加、兄弟追加、ドラッグ配置、チャット取り込み配置で親を自動展開した時も同じカラムへ即時保存する。`localStorage` は折りたたみ状態の正本にしない。
