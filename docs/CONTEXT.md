...
- メモ編集シートの予定化は、日付フィールドからこの画面専用の小さな月間カレンダーPopover、時刻フィールドから24時間/60分の2カラムスクロールホイールPopover、所要時間は `5分` / `15分` / `30分` / `1時間` / `2時間` / `カスタム` のチップで選ぶ。日付Popoverは選択後に閉じる。時刻Popoverは日付と同じくフィールド下側に出し、選択済みの時/分を中央へ寄せ、モバイルで親シートのスワイプ閉じと干渉しない慣性スクロールにする。カレンダー追加欄は `useCalendars` の選択済み/書き込み可能なGoogleカレンダーを優先して表示し、選択後はPopoverを閉じる。候補が無い時だけ `primary` にフォールバックする。予定追加ボタンは日時・所要時間・追加先カレンダーが揃うまで無効にし、選択した `calendar_id` を `/api/wishlist/[id]/calendar` へ渡す。
