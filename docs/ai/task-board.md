...
|---|---|---|---|---|
| TASK-20260611-007 | 2026-06-11 | マップ三点メニューの右編集シート化 | - | 自作マップのノード三点ボタンで開く `CodexNodePanel` を、デスクトップでは右側から出るメモ編集シート、スマホでは下部シートとして表示するようにした。既存の見出し・メモ詳細・画像・Codex・時間・タグ編集を同じUIで維持し、Codexボタンは白黒の `Codexに送る`、保存は黄緑系の `保存` に整理した |
