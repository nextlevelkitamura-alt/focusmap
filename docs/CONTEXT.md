...
- `Todo` タブ左側のメモカードは、`メモ` 画面と同じ `WishlistCardDetail` 編集シートを開く。見出し・本文・タグ・画像・予定化などのメモ編集導線は左ペインからも同じ挙動にする。狭幅ブラウザでも `useIsMobile` はモバイル幅判定でスマホUIに切り替える。スマホの通常メモ編集は黒背景の下部シートを正にし、上部ハンドル、`メモを追加` 見出し、右上の追加先セレクト（`メモ` / `今日`）と×、`見出し`、`メモの内容`、`所要時間`、`画像`、強い緑の `保存` だけを表示する。通常のスマホ編集シートにはCodex、タグ、日付、時刻、カレンダー、構造化パネル、対話履歴を出さない。シートを開いた直後は入力欄へ自動フォーカスせず、キーボードを出さない。スマホでは上端から下へ引いた時も閉じる。
...
- `メモ` 画面の `+ 追加` は、入力欄に本文がある時はその先頭行をタイトルにして即時メモ追加し、入力欄が空の時はタイトル空欄の未保存ドラフトとして新規メモ詳細シートだけを開く。空ドラフトは見出し入力を自動フォーカスせず、`新しいメモ` を初期値として表示しない。見出し/本文/画像/タグ/予定など保存対象の内容が入った時だけ作成POSTを走らせ、何も入力せず閉じた場合はローカルドラフトを破棄してサーバーへ作成しない。メモ詳細はPCでは見出し・本文を自動保存し、スマホでは強い緑の `保存` で明示保存する。作成POSTは15秒タイムアウト付きでリトライし、サーバー側は同じクライアントUUIDの再送を既存メモ取得として扱う。これにより通信が固まった場合でも再送で二重作成しない。仮メモの `display_order` はクライアントから作成APIへ送らず、サーバー側でPostgreSQL `integer` 範囲内の値だけを採用し、それ以外は件数ベースで採番する。`生成` はAI整理追加の専用導線にし、純粋追加と分ける。新規メモ/AI生成結果はタグを自動で入れず、タグ欄は空の状態から始める。クイック追加・AI整理から作ったメモも同じ詳細シートを開く。メモ一覧取得と事前読み込みは `selectedSpaceId` だけでなく `selectedProjectId` も渡し、選択中プロジェクトへ追加したメモが同じ画面に即表示されるようにする。
- デスクトップ通常 `メモ` 画面の看板ボードは `メモ` / `今日` / `完了` の3列に絞る。`メモ` は横幅を広く取り、左パネル表示中はカードを2列、左パネルを閉じた時はカードを最大3列グリッドで表示する。`今日` と `完了` も狭くしすぎず、カード本文が読める最低幅を確保する。通常メモ画面はヘッダー下から画面下端までを1つの作業面として使い、外側のページスクロールではなく、左の追加/編集パネル本文と各レーン内だけをスクロールさせる。左パネル、`メモ` / `今日` / `完了` レーンの枠は同じ高さで下端まで伸ばし、保存ボタンは左パネル下端の固定フッターに置く。予定済み・マップ追加済みのDB状態は壊さず、今日/完了でないアクティブメモは通常看板上では `メモ` に集約し、カード内の既存チップで予定済み/マップ追加済みを示す。`メモ` と `今日` の見出し右側には小さな＋を置き、押すと左の追加パネルの追加先をその列に合わせる。`完了` には追加＋を置かない。メモカードは左上にプロジェクト、見出し下に重複排除したタグ、右上に完了チェックを置き、今日するアイコンと一覧カード右側のCodex/クリップボード導線は表示しない。スマホ看板では `メモ` / `今日` / `完了` の横スナップだけを通常表示し、カードを左右端へ短く保持すると隣の看板へスクロールし、ドロップ時にカラム移動として保存する。タグ絞り込みとメモ選択マップ化の処理はコード上に残すが、通常UIの上部ツールバーからは非表示にする。タグ候補は編集シート内でスマホでも押しやすいチップサイズを維持する。
- モバイル `メモ` の未予定/今日カードには左側に小さな `予定に入れる` を置き、削除はカード右上寄りのゴミ箱にする。タップ時は `Todo` タブへ遷移せず、メモタブ内で一時的なDayカレンダー（`Day` / `3days` / `Month` 切替つき）を表示する。予定ブロック全体をドラッグし、離した時刻で `/api/wishlist/[id]/calendar` に保存する。保存処理は `handleMemoCalendarDrop` / `handleCalendarAdd` を通し、`useCalendarEvents` の optimistic broadcast とカレンダーキャッシュ無効化で `Today` 側も同時に反映する。確定ボタン、キャンセルボタン、6点ドラッグハンドルは出さず、drop後もカレンダー画面を維持し、左上の戻るボタンでメモ一覧へ戻る。
