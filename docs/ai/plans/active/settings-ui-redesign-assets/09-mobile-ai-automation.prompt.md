...
Canvas: iPhone-sized portrait mockup, black/white/gray Focusmap UI.
...
- Collapsible "導入/再設定"
- Inline monochrome notice for stale state
State requirements: normal connected state plus one "要確認" row with label and hierarchy, not bright color.
Responsive note: all tap targets at least 44px; no horizontal tables.
Text: Japanese concise labels, no long paragraphs.
Visual style: mobile Focusmap black/white/gray control surface, not a settings brochure. Use smaller modern mobile settings typography and monochrome status labels.
Constraints: no fake logos, no decorative blobs, no tiny diagnostic logs, no nested cards, no oversized button blocks, no emoji icons, no bright status color palette.
```
