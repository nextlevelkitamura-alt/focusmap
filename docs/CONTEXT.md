...
- チャット取り込み詳細ヘッダーは、Codex thread titleを主見出しにし、状態チップは見出し右側へ置く。repo/project名・配置状態・更新時刻は見出し下のメタ行にまとめ、その下に `AI要約` の3カラム（やったこと / 変更・判断 / 次に見ること）と `Codexで開く` 導線を置いてから原文ログを表示する。AI要約は保存済みactivityと取り込みsnippetから生成し、activity同期中も空欄にしない。
