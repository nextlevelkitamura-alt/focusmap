...
- スマホ/マップUIのCodex進捗表示は `/api/task-progress/snapshot?updated_after=<cursor>` を3〜5秒間隔で読む。初回はcursorなしで最大500件、以後は返却された `cursor` 以降の差分だけ取得する。Mac agentはCodex.appのローカル通知を1〜2秒単位で拾うが、Tursoへの通常progress送信は内容hashが変わった時だけ、かつ最短3秒間隔にする。thread作成・プロンプト送信・確認待ち・完了・失敗は即時送信する。
