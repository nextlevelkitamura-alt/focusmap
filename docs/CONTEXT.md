...
- チャット取り込み詳細ヘッダーは、Codex thread titleを主見出しにし、状態チップは見出し右側へ置く。repo名・配置状態・更新時刻は見出し下のメタ行にまとめ、project名チップは一覧カードや詳細ヘッダーの主要表示へ出さない。その下に `AI要約` の3カラム（やったこと / 変更・判断 / 次に見ること）と `Codexで開く` 導線を置いてから原文ログを表示する。AI要約は保存済みactivityと取り込みsnippetから生成し、activity同期中も空欄にしない。Focusmap Macアプリでは `Codexで開く` が `openExternal` bridge 経由で該当 `codex://threads/<thread id>` をOSへ渡し、通常ブラウザでは同じURLのリンクとして開く。
