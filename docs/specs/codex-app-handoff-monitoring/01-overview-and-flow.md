...
Codex.app 全履歴を取り込むAI履歴サイドバーでは、通常表示の分類を2つに固定します。

```text
ai_history_items.source_task_id is null     -> 未配置
ai_history_items.source_task_id is not null -> マインドマップ
ai_history_items.archived = true            -> 通常UIから完全非表示
```

- `未配置` をdefault viewにし、`マインドマップ` は補助viewにする。
- archived rowは物理削除しない。Codex.app側でarchive解除され、metadata reconcileで `archived=false` が届いたら同じhistory itemを復元する。
- archive前に `source_task_id` があり、対象taskがまだ存在する場合は、復元後も `マインドマップ` 配置を保持する。
- repo selectorは表示フィルタだけにする。`全体` は選択中projectに紐づく有効repo scopeを横断表示し、個別repo選択は同じmetadata一覧を絞り込むだけで、同期scopeの追加・削除・ON/OFFは行わない。
- サイドバーの上ヘッダーは `AI履歴`、表示専用の `AI online` / `AI offline`、repo表示フィルタ、設定アイコン、閉じるだけにする。右上トグル、検索欄、大きいrepo監視ブロック、sync文言、import/Finder/Codex boardボタンは置かない。
- `AI online` は機能が有効でlocal agentが接続/runningしている状態表示であり、sidebar上のユーザー操作トグルではない。詳細設定と診断は設定画面へ寄せる。
...
- スマホ/外部アプリの `open` / `blur` / `visibilitychange` / 復帰は、送信や実行の確認として扱わない。thread未検出のmanual handoffは `未送信` のまま残し、Mac local monitorや `/api/codex/sync-node` がCodex.app thread、`task_started`、直近のユーザー可視Codex activity、またはauto dispatchのqueue/run activityを確認した時だけ `実行中` へ進める。
...
- AI履歴一覧はmetadata-onlyにし、本文/full activityは詳細を開いた時だけ取得またはhydrateする。
- AI履歴metadataの同期cursorは `indexed_at|id` を使う。Codex側の `last_activity_at` は並び順用であり、cursorには使わない。
- AI履歴のarchive観測は通常UIから隠すだけで、単体ではFocusmap source node完了や `ai_tasks.completed` の根拠にしない。Focusmap側のpending archive requestに対する完了確認だけが既存manual handoffの完了同期根拠になる。
