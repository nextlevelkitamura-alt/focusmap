...
- メモに画像添付がある場合でも、Codexへコピーするprompt本文には `添付画像:`、画像名、形式、サイズ、署名URLを追加しない。prompt本文は「見出し + 1改行 + 本文」だけを基本にし、画像バイナリをdeep linkへ詰め込まない。保存済み画像はCodex欄の画像コピーアイコンから個別にOSクリップボードへ保存し、ユーザーが同じCodex入力欄へ続けて貼り付ける。複数画像もprompt本文へURL一覧を入れず、UIから各画像を必要な順にコピーする。
