...
/**
 * モデル選定:
 *   - DEEPSEEK_API_KEY 設定時: DEEPSEEK_AUTOMATION_MODEL / DEEPSEEK_MODEL
 *     (デフォルト deepseek-v4-pro) を使う
...
function getModel() {
  if (process.env.DEEPSEEK_API_KEY) {
    const modelId = process.env.DEEPSEEK_AUTOMATION_MODEL ?? process.env.DEEPSEEK_MODEL ?? 'deepseek-v4-pro';
...
export function getActiveModelLabel(): string {
  if (process.env.DEEPSEEK_API_KEY) {
    return process.env.DEEPSEEK_AUTOMATION_MODEL ?? process.env.DEEPSEEK_MODEL ?? 'deepseek-v4-pro';
  }
