...
- 実装上の正本は、DBが `mindmap_drafts` / `mindmap_draft_nodes` / `mindmap_draft_history`、APIが `/api/mindmap/drafts`、`/api/mindmap/drafts/[draftId]/nodes`、`/api/mindmap/drafts/[draftId]/apply`、`/api/mindmap/draft-history/[historyId]/undo|redo`。AIツールは `saveMindmapDraft` を下書き保存専用に使い、`proposeMindmapOrganization` は読み取り専用にする。マップUIは `mindmap_drafts` / `mindmap_draft_nodes` のRealtimeと `focusmap:mindmap-draft:changed` イベントでactive draftを再読込し、AI案上の手動追加・移動・タイトル変更は下書きノードAPIへ保存する。確定時に `useUndoRedo` へDB履歴ベースのUndo/Redo actionを登録し、Cmd/Ctrl+Z系の既存導線へ接続する。
- `マップチャット` と `AI案` 下書きの実装分割、DB案、モックアップは [docs/plans/active/mindmap-map-chat-ai-draft.md](plans/active/mindmap-map-chat-ai-draft.md) を参照する。
