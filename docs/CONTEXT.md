...
- `Codexに送る` 直後のmanual handoffは `未送信` として表示する。Codex.appのthreadを検出しても、Codex出力/進捗文がまだ無い間は `未送信` のままにする。`codex_thread_snapshot.preview` / `message` / `current_step` などユーザー可視のCodex出力が取れたら `未送信` から外し、threadの `updated_at_ms` や `last_activity_at` が直近45秒以内なら `実行中`、動きが止まっていれば `確認待ち` とする。
...
- ノード詳細を開いている間は、`未送信` / `実行中` / `確認待ち` のCodex taskだけ `/api/codex/sync-node` を約3秒ごとに呼び、Codex.appへの貼り付け・出力開始・出力停止を短時間でUIへ反映する。ただし `/api/codex/sync-node` のTurso mirrorは、thread検出、状態変化、Codex thread `updated_at_ms` 変化、current_step変化がある時だけ行い、同じsnapshotを3秒ごとにTursoへ書かない。
...
- 実行中はMac側で `~/.codex/state_5.sqlite` / rollout JSONL / app-server通知を1秒単位で確認してよい。ただしTurso/Supabaseへは毎秒書かず、通常running snapshotは5秒を最短、AI詳細panel/drawerのactive watch中だけ3秒を最短にする。同一hashなら送信せず、thread検出・running再開・確認待ち・完了・失敗など状態変化だけforce送信する。
...
- DBにはCodexの全生ログを保存しない。Supabase互換の `ai_tasks.result.live_log` は表示用tailに抑え、Tursoは `ai_tasks` latest snapshot、`ai_task_progress` の短いtail、`ai_task_events` の状態変化だけを持つ。`raw_log` / `output` / full thread history / image body はTurso payload sanitizerで落とし、チャット風の活動記録は必要最小限を `ai_task_activity_messages` またはTurso progress/eventへ残す。
