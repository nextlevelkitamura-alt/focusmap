...
## 配信・信頼性設計（2026-05-30 壁打ち確定）

前提: FocusMap は **デプロイ済サイト(https)主** で使う。よって**ブラウザ→localhost直送は不可**（mixed content/CORS）。
配信は常に Supabase 経由。当初要望の「PCから直接ローカル送信」「完全オフライン実行」は deployed-site では原理的に不可能
（実現するならローカル FocusMap を使う別選択。今回は見送り）。本設計は制約下の最善＝「速い＋確実な Supabase 経路」。

**確定事項:**
1. **ピックアップ = Realtime 即時 + ポーリング保険**
   - task-runner を 15〜18秒ポーリングから **Supabase Realtime 購読**（ai_tasks INSERT を即push, <1s）へ。
   - 低頻度ポーリングを保険で残す（接続断・スリープ復帰の取りこぼし防止）。egress も削減（通信制限と相性良）。
2. **送信信頼性 = 軽いリトライ（完全オンライン前提）**
   - 【方針更新 2026-05-30】**Codex自体がオンライン必須**（OpenAIバックエンド）なので、オフライン対応は無意味と判断。
     永続 offline outbox(IndexedDB) は**作らない**。完全オンライン前提。
   - 送信は `/api/ai-tasks/schedule` へ POST。失敗時は**数回の軽いバックオフ再試行**のみ（Supabase の一時 throttle/fetch failed 吸収）。
     それでも失敗ならエラー表示。成功で1回確定（重複なし）。
3. **送達確認 = Mac が claim した時点**
   - 「クラウドに入った」では未達扱い。**Mac が拾って status=running になったら『達』**。
   - それまで UI は「Mac待ち（未達）」表示。再POSTはしない（重複防止）。durable なので落ちない。
4. **ステータスモデル**: `送信待ち(outbox) → クラウド受領 → Mac待ち(未達) → 実行中 → 完了/承認待ち`
5. **スリープ/オフライン復帰**: 未達は Supabase に滞留 → 復帰時に Realtime再接続＋ポーリングで拾う。
   - **直列実行（1件ずつ）**＝全開Codexの一斉暴発を防ぐ。
   - **TTLなし（期限なし・いつでも実行）**をユーザーが選択。⚠️古い指示が後で走るリスクは受容済み（直列で緩和）。
6. **通知**: FocusMap 内ステータスのみ（Mac通知/スマホpushは無し）。

**前提となる修正（信頼性の土台）:**
- task-runner の **per-task エラー隔離**が必須。現状 Claude タスクの `spawn ENOEXEC` で runner プロセスが丸ごと落ち、
  codex ジョブが巻き添えで処理されない（実測）。1件の失敗で全体が落ちないようにするのが本設計の前提。
- Supabase egress/通信制限は環境要因。Realtime化で問い合わせを減らし緩和するが、根本は Supabase 側の確認が要る。

**実装対象（次フェーズ・完全オンライン前提）:**
- [ ] task-runner: per-task try/catch（resilience）← 最優先・前提（現在の ENOEXEC 全体クラッシュを解消）
- [ ] task-runner: Supabase Realtime 購読 + ポーリング保険、直列実行キュー
- [ ] FocusMap: 送信は軽いリトライのみ（永続outboxは作らない）
- [ ] FocusMap: 配信ステータス表示（送信待ち/クラウド受領/Mac待ち/実行中/完了）
