...
- モバイル `マップ` ヘッダーの左上に独立した `マップ` 見出しは出さず、`SpaceProjectSwitcher` の `全体` / プロジェクト選択を左詰めで最初の情報にする。マップ画面内のCodex入口は右上Botアイコンだけにし、下部の `Codex online` フローティングボタンは出さない。
...
- スマホのCodexシートは `取り込み` と `看板` の2タブに分ける。`取り込み` は選択中projectの `Codex Inbox` 配下にある `source='codex_app_thread'` ノードのうち、repo pathが一致し、かつ `未送信` / thread未検出ではないものだけをカード表示する。カードにはCodex thread title由来のタイトル、`tasks.memo` の `## 初回依頼` から抽出したプロンプト本文、repo名、thread id短縮、状態、最終更新を出し、`配置先を選ぶ` でシートを閉じてマップ上の配置モードへ入る。配置モード中はノードタップで編集を開かず、選んだノードの子として取り込みノードを移動し、親ノードが折りたたまれていれば展開する。
