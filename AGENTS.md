...
- worktree / feature / fix ブランチにだけ作業コミットが残っている状態で終わらない。local main へ取り込めない事情がある場合だけ、`main未統合` と理由、対象branch、commit hash、次に必要な判断を報告する
- local main が `origin/main` より先行している状態は許容する。ただし完了報告では `local main: 取り込み済み / origin/main: 未push / 本番: 未反映` のように分けて書く
...
### UIビジュアル統一（必須）
- スマホ・PC・Macアプリ・iOSアプリは、同じFocusmapに見える色味・状態色・アイコン・線幅・角丸を使う
- 画面や端末ごとに別アプリのように見える独自パレット、独自アイコンセット、別系統の装飾を持ち込まない
- アイコンは原則として既存の `lucide-react` と共通UIコンポーネントを使い、サイズ・stroke・色の扱いを揃える
- プラットフォーム差は配置、密度、safe area、入力方式で吸収し、ブランドの視覚言語は共通デザイントークンを優先する
...
- チャット画面でエージェントが予定作成・メモ追加・マインドマップ整理・タスク更新などを実行した場合も、完了返答と同時に対象UI（カレンダー、メモ、マップ、Todoなど）へ即時反映する。チャットだけに「了解しました」と残して対象画面の更新を次回手動更新待ちにしない
