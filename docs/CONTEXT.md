...
- スマホCodexシートの取り込み一覧は、Codex監視画面から遷移する `AIチャット履歴` として表示する。左上は三本線や新規チャットではなく、矢印アイコン付きの小さな日本語 `戻る` ボタンにし、右側は `Codex` とMac接続状態にする。履歴は `すべて` / `確認待ち` / `実行中` / `完了` / `接続失敗` の横スクロールタブで絞り込む。各履歴カードはthread IDを出さず、`履歴を見る` で同じCodexシート内のチャット/activity詳細へ切り替える。詳細面は通常チャットに近い黒基調の会話表示と下部入力欄モックで、詳細左上の `戻る` はマップではなく `AIチャット履歴` 一覧へ戻す。マップ配置は既存の `配置先を選ぶ` を残すが、履歴閲覧の主導線は外部CodexリンクではなくFocusmap内のactivity表示にする。
