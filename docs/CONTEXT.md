...
- `CodexNodePanel` は `メモを編集` のフル編集パネルとして扱うが、可視タイトルは出さずコンパクトな編集シートにする。デスクトップでは画面右から `w-[min(92vw,460px)]` 程度のシートで開き、スマホでは下部シートで開く。上部固定の見出し領域は置かず、本文スクロール内で `見出し → メモ詳細 → 予定 → Codex → 画像` の順に並べる。見出しは1行入力、メモ詳細の見出し行には音声入力アイコンを常時置き、本文が入った時だけSparkles付きの `見出し生成` を出して `/api/ai/generate-memo-heading` を呼ぶ。タグは表示しない。保存ボタンは下部に白背景の `保存` として置く。スマホの画像追加は「画像を追加」だけを出し、クリップボード貼り付けはPC導線に寄せる。画像は `/api/tasks/[id]/attachments` でフォルダー選択、ドラッグ&ドロップ、クリップボード貼り付け、削除を行う。保存済み画像は添付カード内のコピーアイコンからCodex貼り付け用にコピーでき、カード本体クリックでプレビューを開く。`task_attachments` APIはユーザーセッションで対象タスクと添付の所有確認を行い、その後のStorage操作と添付DB操作はserver-side service role clientで実行する。アップロード中は元画像のローカルプレビューを薄く表示し、オンライン送信前にクライアント側でJPEGへ圧縮して300KB以下にする。保存済み添付に入るまで、またはMac heartbeatが未確認/offlineの間はCodex送信を無効にし、サーバー側も300KB超の画像を拒否する。予定ブロックには時刻入力を出さず、所要時間は15分を既定値にしつつ `5分` / `15分` / `30分` / `1時間` / `2時間` / `カスタム` から選ぶ。カレンダーは `useCalendars` の選択済み/書き込み可能なGoogleカレンダーを優先し、候補が無い時だけ `primary` にフォールバックする。`予定を入れる` は所要時間とカレンダーが揃った時だけ有効にし、`estimated_time` / `calendar_id` を保存して `focusmap:open-today-calendar` でToday予定画面へ移動する。ここでは `/api/calendar/sync-task` を直接呼ばず、時刻配置はカレンダー側で行う。
