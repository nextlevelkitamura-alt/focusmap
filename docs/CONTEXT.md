...
- メモ詳細の `Codexにプロンプトを送る` は標準では `executor='codex_app'` / `dispatch_mode='manual'` の `ai_tasks` を先に作り、Focusmap同期ID（handoff token）付きpromptをクリップボードへコピーしてCodex.app/ChatGPT Codex入口を開く。Codex.appで最終送信するのは人間で、Focusmapはprompt / handoff token / ai_task tracking packageを作る。自動 `thread/start` / `turn/start` は、明示的な自動実行導線が `dispatch_mode='auto'` を渡した時だけ使う。
...
- `Codexに送る` 直後のmanual handoffは `未送信` として表示する。Codex.appのthreadを検出しても、Codex側で送信・活動が確認できるまでは `未送信` のままにする。threadの `updated_at_ms` や `last_activity_at` が直近の5秒pulseで動いていれば `実行中`、Mac heartbeatはあるがCodex activityが止まっていれば停止疑いまたは `確認待ち` とする。ユーザー可視の出力本文やpreviewは通常snapshotへ保存しない。
...
- ノード詳細を開いている間は、`未送信` / `実行中` / `確認待ち` のCodex taskだけ `/api/codex/sync-node` を約5秒ごとに呼び、Codex.appへの貼り付け・稼働開始・停止を短時間でUIへ反映する。`/api/codex/sync-node` は、thread検出、状態変化、Codex thread `updated_at_ms` / `last_activity_at` 変化、current_step変化、一定間隔の `codex_last_checked_at` 更新がある時だけSupabase/Tursoへ書き、同じsnapshotを5秒ごとに保存しない。
...
- 実行中はMac側で `~/.codex/state_5.sqlite` / rollout JSONL / app-server通知を短周期に確認してよい。看板・一覧向けの通常snapshotは5秒ごとの軽量pulseを最短にし、`runner_heartbeats.current_task_id`、`last_seen_at`、`ai_tasks.last_activity_at/current_step/summary` の短い状態だけを持つ。同一hashや同じ活動時刻なら送信しない。thread検出・running再開・確認待ち・完了・失敗など状態変化だけforce送信する。
- メモからCodexへ渡す導線と活動表示の詳細計画は `docs/specs/memo-codex-execution/requirements.md` / `delivery-plan.md` を参考にするが、標準UIはmanual handoffを正とする。確認待ち後にCodex側で追加プロンプトが送られた場合は、Mac localで5秒以内に再開を検知して `running` snapshot/eventへ戻す。
- DBにはCodexの全生ログやfull thread historyを保存しない。看板・一覧の通常running同期では `ai_tasks.result.live_log` / `output` / Codex thread preview を保存せず、Turso/Supabaseのlatest snapshotは軽量pulseに寄せる。一方で、ノード/メモ詳細を開いた時にチャットとして確認できるよう、Codex側の最新ユーザー可視発話・質問・確認内容だけは `ai_task_activity_messages` またはTurso progress/eventへ短文で残す。`current_step` は看板向けに `Codex.appが作業中です` / `確認待ち` など状態語に寄せ、会話本文は詳細API `/api/ai-tasks/[id]/activity` で読む。
- Codex thread未検出の高速探索は開始後2分まで。2分を超えて見つからない場合は `monitoring_lost` として確認待ちにする。
- 確認待ち・手動貼り付け待ち・needs_inputは、Mac側ではCodex.app sqlite/rollout/app-serverを1秒単位で確認してよい。ただしDBへは無変化の `codex_last_checked_at` だけを書かず、人間がCodex.appへ追加入力して `running` に戻った時、thread検出、確認待ち化、完了/失敗などの状態変化だけを5秒以内にsnapshot/eventへ反映する。
...
- `/api/ai-tasks` の一覧取得は `select('*')` を使わず、一覧・バッジ・タイムラインに必要なカラムと `result` のJSON pathだけを取得する。巨大な `result` / `codex_thread_snapshot` / 生ログを一覧レスポンスへ載せない。詳細ログが必要なメモ詳細・CodexNodePanel・progress詳細panelだけ `/api/ai-tasks/[id]/activity` を5秒pollで取り、Focusmapから送ったpromptとCodex側の発話をチャットとして表示する。activityがTurso progress/eventだけの場合も、送信promptを先頭のユーザー発話として補う。
- Web側の `useAiTasks` / `useMemoAiTasks` / `useNoteAiTasks` / `useScheduledTasks` は、広域の `ai_tasks` Realtime購読をやめ、Bearer付きREST snapshot取得へ寄せる。`running` のCodexタスクだけ5秒更新、`pending` は30秒、確認待ち・完了・失敗・手動貼り付け待ちは低頻度または手動更新にする。ブラウザ/iPhoneがバックグラウンド中はinterval取得しない。ローカル `/api/codex/sync-node` は手動sync-now用途で、Turso snapshot/eventへも軽量mirrorする。
...
- Macアプリ/ローカルWebのメモ詳細では、Codex.appへの自動 `thread/start` / `turn/start` を標準導線にしない。Focusmapはprompt / handoff token / ai_task tracking packageを作り、クリップボードコピーとCodex.app起動まで補助する。人間がCodex.appで送信した後、Electron/常駐runner側が `ANTHROPIC_API_KEY` / `CLAUDECODE` を外した環境でローカルsqlite/rollout/app-serverを監視し、Turso snapshot/eventへ同期する。明示 `dispatch_mode='auto'` の互換経路は残すが、通常UIはmanual handoffを正とする。
