...
- デスクトップの `マップ` 画面では、上部中央の旧 `AIで整理` ボタンを置かない。右上操作は `AI実行` の右側に `マップチャット` ボタンを置き、その右にカレンダー/メモ等の右ペイン操作を並べる。`マップチャット` はカレンダー/メモ右ペインと同じ排他枠を使い、押すとマップ右側のリサイズ可能なサイドバーに選択中プロジェクトの `UnifiedChat` を表示する。サイドバー見出しは `マップチャット` とし、会話本文・スターター・履歴/新規チャット操作は通常の `UnifiedChat` と共通にする。選択プロジェクトを変えた場合は、開いているPCサイドバーのプロジェクトチャットも新しいプロジェクトへ追従する。
