...
- `Todo` タブ左側のメモカードは、`メモ` 画面と同じ `WishlistCardDetail` 編集シートを開く。見出し・本文・画像・予定化などのメモ編集導線は左ペインからも同じ挙動にし、タグ入力UIは置かない。狭幅ブラウザでも `useIsMobile` はモバイル幅判定でスマホUIに切り替える。スマホの通常メモ編集は黒背景の下部シートを正にし、上部ハンドル、`メモを追加` 見出し、右上の追加先セレクト（`メモ` / `今日`）と×、`見出し`、`メモの内容`、`Codexに送る`、`所要時間`、`画像`、白背景の `保存` を1画面内へ収める。スマホ追加シートのCodex送信は保存とは別の明示ボタンで、メモ本文/見出しあり、プロジェクトとrepo設定あり、Mac heartbeat online、画像保存待ちなしの時だけ有効にする。通常のスマホ編集シートにはタグ、日付、時刻、カレンダー、構造化パネル、対話履歴を出さない。シートを開いた直後は入力欄へ自動フォーカスせず、キーボードを出さない。スマホでは上端から下へ引いた時も閉じる。
- メモ編集シートの本文欄は、同じ枠の中を本文スクロール領域と下部の保存状態表示に分ける。PCでは `本文を音声入力` と `本文から見出し生成` をメモ詳細見出し行の右側、コピーアイコンの左側へ小さく並べ、コピーは文言を出さずアイコンだけにする。スマホの下部編集シートでは、見出し入力欄にSparklesボタンを置かず、`メモの内容` 欄の右下にSparklesの本文から見出し生成、その下に音声入力を縦に近接配置する。本文テキストは右下アイコン領域を避けて折り返し、本文が長くなってもスクロールは本文領域だけで発生する。音声入力は `useVoiceRecorder` と `/api/transcribe` を使い、文字起こし結果を本文へ追記する。見出し生成は `/api/ai/generate-memo-heading` を使い、本文から14〜22字程度、長くても24字以内の短い日本語見出しを作る。
- メモ詳細の見出し/本文は、入力中のローカルdraftを端末側の正として扱い、DB保存完了前に古いDB値で入力内容を巻き戻さない。`WishlistCardDetail` はメモIDごとに `localStorage` へ未同期draftを保存し、同じメモを開き直した時にDB値と異なるdraftを復元する。PCは従来通り短いdebounceで自動保存するが、表示は `端末に保存済み` / `同期中` / `保存済み` / `未同期・再試行` に分け、DB ACK前に保存済み扱いにしない。スマホは白背景の `保存` を押した時だけDBへ反映し、保存失敗時はシートを閉じず、端末draftを保持して近くにエラーを表示する。
- メモ編集シートの `画像` セクションは本文の直下に常時表示し、同じ添付API `/api/wishlist/[id]/attachments` を使う。スマホでは中央の「写真を選択」タイルだけを追加導線にし、見出し行右上の追加ボタンとクリップボード貼り付けボタンは出さない。スマホのサムネイルは横スクロールではなく2列グリッドで表示し、メモ編集シート全体は横方向へスクロールさせない。PCでも画像見出し右の追加ボタンは出さず、見出し/プロジェクトの下を「左: メモ詳細」「右: 画像 → 時間・予定 → Codex」の2ペインにし、画像・予定入力がメモ詳細の左へ戻らないようにする。画像・時間・Codexの各セクションは、メモ欄と同じく見出しを外側、操作面を内側の枠として表示する。PCではクリックでフォルダーから選択、ドラッグ&ドロップ、`クリップボード画像を貼り付け` ボタン、画像セクション内またはメモ編集シート内の `Cmd/Ctrl+V` で画像を追加できる。サムネイルは追加タイルより上に置き、アップロード中は元画像のローカルプレビューを低透明度で即時表示し、オンライン送信前にクライアント側でJPEGへ圧縮して300KB以下にする。登録完了後に通常の濃さと保存/削除操作へ切り替える。複数画像は並列アップロードし、仮メモ作成中に追加された画像はメモ作成APIの完了を待ってから添付APIへ送る。添付APIはユーザーセッションで対象メモの所有確認を行い、その後のStorage操作と `ideal_attachments` の取得/作成/削除だけをserver-side service role clientで実行する。これにより本番DBのRLS policy差分で本人の画像添付が `new row violates row-level security policy` に落ちる状態を避ける。サーバー側も300KB超の画像を拒否し、画像アップロードは60秒でタイムアウトし、失敗時だけ対象の仮プレビューを消してエラーを表示する。画像削除はDELETE完了を待たずにサムネイルを即時非表示にし、失敗時だけ元の位置へ戻す。添付画像のStorage保存パスはISO時刻トークンを使い、ミリ秒の巨大整数をDB値と誤認させない。
...
- スマホCodexシートの取り込み一覧は、Codex監視画面から遷移する `AIチャット履歴` として表示する。左上は三本線や新規チャットではなく戻る矢印、右側は `Codex` とMac接続状態にする。履歴は `すべて` / `確認待ち` / `実行中` / `完了` / `接続失敗` の横スクロールタブで絞り込む。各履歴カードはthread IDを出さず、`履歴を見る` でFocusmap内のチャット/activity詳細へ遷移する。マップ配置は既存の `配置先を選ぶ` を残すが、履歴閲覧の主導線は外部CodexリンクではなくFocusmap内のactivity表示にする。
