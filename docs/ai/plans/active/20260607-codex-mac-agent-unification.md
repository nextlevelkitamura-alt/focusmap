...
- 2026-06-23: Agent worker commit `3dd46d1f` をlocal mainへ統合した。`focusmap-agent` はrepo scan結果を `available_repo_keys` / `repo_paths` としてheartbeatへ載せ、browser / terminal / playwright / simple のrecurring taskを完了後 `pending` + next `scheduled_at` へ再投入できる。Agent state APIは `next_scheduled_at` / `completed_at` を受け取り、claim解除・error解除込みで再投入する。staff-status、Claude系、package実行、Codex.app recurring taskの承認/完了後再投入は通常agentへ無理に混ぜず、legacy/debugまたは後続判断として残す。次はDesktop/Installer workerで `setup.sh` / 旧plist / Mac app status から旧 `task-runner` 通常導線を外す。
- 2026-06-23: Desktop/Installer worker commit `07665c06` をlocal mainへ統合した。Mac appのmanual-connect / supervisor / `ensureAutomationServices()` は旧 `task-runner` を通常復旧せず、接続判定から旧runner pause状態を外した。`scripts/setup.sh` は旧plistを新規copy/loadせず、既存plistがある場合は停止・削除案内だけを出す。`scripts/install.sh` は既存の旧launchd labelを停止し、`~/Library/LaunchAgents/com.focusmap.task-runner.plist` も削除する。旧plist本体は `legacy/debug` 専用資料として残す。次はIntegrationで全差分レビュー、docs最終確認、検証コマンド候補、3秒測定/実機確認の実行可否をまとめる。
- 2026-06-23: Integration workerでlocal main上のAPI / UI / Agent / Desktop-Installer成果を確認した。通常APIから `task-runner.ts --fast` spawnは残っておらず、Mac appの接続判定はNext / `focusmap-agent` / Codex app-serverを正にする。`setup.sh` は旧plistをcopy/loadせず、`install.sh` は旧launchd停止とplist削除を行う。UIのactive Codex表示は3秒read refreshで、表示中を理由に `/api/codex/sync-node` をPOSTしない。追加で `/api/available-repos` と `/api/codex/open-repo` のrepo候補/許可判定を agent heartbeat の `ai_runners.repo_paths` 正本へ補正し、旧 `available_repos` は互換fallbackにした。3秒同期は静的/テスト確認までで、Mac実機の5回測定は未実行。残タスクはstaff-status、Claude、package、Codex.app recurring再投入の通常移管またはlegacy/debug維持判断。
