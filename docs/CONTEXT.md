...
- デスクトップマップ右上のCodexロゴボタンは `チャット取り込み` 右サイドバーを開く。サイドバー表示中は右上のCodex/設定ボタンを隠し、サイドバーはマップ領域の上下右端に隙間なく接地させて下部の `Codex看板` と重ねない。サイドバー内には見出しの `チャット取り込み` 文言を出さず、先頭から監視・リポ選択・チャット一覧を表示して一覧領域を広く取る。サイドバーはproject単位ではなく選択中のリポ単位の未配置Inboxとして扱い、Mac側 `focusmap-agent` が登録した `available_repos` 候補またはFinder選択で対象リポを切り替える。通常UIには長いパス手入力欄や常時表示の候補リストを出さず、`リポ監視` 行に対象リポ名と監視中project名を小さく出し、既存リポ候補は `既存リポ選択` のポップアップ、任意フォルダはFinder導線から選ぶ。`リポ監視` は選択リポに対して有効な `projects.codex_thread_import_enabled` scopeがあるかを表示し、ONにする時は現在projectの `projects.repo_path` を対象リポへ合わせてscopeを作る。OFFにする時は同じリポの有効scopeを止める。サイドバー下部は選択リポの `source='codex_app_thread'` かつ `codex_work_dir` が一致し、どこかの `Codex Inbox` グループ直下に残っている未配置チャットのうち `実行中` / `確認待ち` / `接続失敗` / `完了済み` に進んだものだけを表示する。現在project外に取り込まれた同一リポのチャットも一覧に出し、チャット行を現在マップのノードまたはproject rootへドラッグすると、既存taskの `project_id` と `parent_task_id` を現在マップへ更新して配置する。配置後は未配置一覧から消える。チャット行クリックでその1件のtask詳細だけを `/api/tasks/[id]` から取得して開き、一覧表示のために全文や詳細tailをバックグラウンドで常時取得しない。チャット行の削除は既存taskを削除し、グローバルUndoで削除前task snapshotを復元できる。UIはCodex sqlite/rolloutを直接読まず、取り込み済みノードとtask-progress snapshotの読み取りだけで一覧を作る。配置済みチャットは複製しない。
- モバイルの `取り込み` タブでは、下部トレイのチャットカードを長押しするとカードをリフトし、シートを下端の小さなトレイへ縮めてマップを広く見せる。ドラッグ中はノードに近づくと対象ノードを強調し、dropで既存taskの `project_id` と `parent_task_id` を現在マップへ更新して、そのチャットを対象ノードの子ノードとして配置する。drop後はチャットカードをトレイから消し、マップ上では通常のノード形状として表示する。titleは取り込み時のthread titleまたは本文1行目、markdown本文は `tasks.memo` に保持し、ノード上には短い本文プレビューとCodex状態バッジだけを出す。新規にCodex threadを取り込む時は、`tasks.memo` に `Thread ID`、`Repository`、最終更新、初回依頼、最新プレビューをMarkdownで保存し、`tasks.codex_thread_id`、`tasks.codex_work_dir`、紐づく `ai_tasks.result.codex_thread_url` / `codex_run_state` も保持する。長押しD&Dに失敗する端末やアクセシビリティ用に、カード詳細から `配置先を選ぶ` を開き、次にタップしたノードへ配置する代替導線も用意する。
...
- スマホのCodexシートは `取り込み` と `看板` の2タブに分ける。`取り込み` は選択中projectの `Codex Inbox` 配下にある `source='codex_app_thread'` ノードのうち、repo pathが一致し、かつ `未送信` / thread未検出ではないものだけをカード表示する。カードにはタイトル、memo先頭、repo名、thread id短縮、状態、最終更新を出し、`配置先を選ぶ` でシートを閉じてマップ上の配置モードへ入る。配置モード中はノードタップで編集を開かず、選んだノードの子として取り込みノードを移動し、親ノードが折りたたまれていれば展開する。
