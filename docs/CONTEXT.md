...
- `チャット` 画面は `UnifiedChat` を正にし、通常会話と実行依頼を1つのチャットへ統合する。画面内に `チャット` / `自動化` のセグメントタブは置かず、予定整理・タスク化・調査などの実行依頼は入力欄左の `+` メニューから同じチャットへプロンプトを挿入する。古い `view=automation` や保存済み `automation` view は `ai` viewへ正規化し、専用の自動化チャット画面へ遷移させない。
...
- Mac接続状態は `チャット` のヘッダーと入力欄上のステータスチップに常時表示する。表示は `Mac online` / `Mac offline` / `Mac未接続` / `確認中` に丸め、online以外では送信ボタンの説明を予約送信として扱う。Mac状態の取得は既存 `useAgentConnection()` / `/api/ai-runners` を使い、チャットUIはrunnerや `ai_tasks` の実行ロジックを直接変更しない。
