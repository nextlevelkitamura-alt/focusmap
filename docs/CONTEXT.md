...
- メモ編集シートの `画像` タブは、同じ添付API `/api/wishlist/[id]/attachments` を使い、クリックでフォルダーから選択、ドラッグ&ドロップ、`クリップボード画像を貼り付け` ボタン、画像タブ内の `Cmd/Ctrl+V` で画像を追加できる。画像追加後も `画像` タブを開いたままにし、追加結果のサムネイル確認と削除を同じ場所で行う。
- `メモ` 画面の `+ 追加` は、入力欄に本文がある時はその先頭行をタイトルにして即時メモ追加し、入力欄が空の時だけ空の新規メモ詳細シートを開く。どちらもクライアント側で先にUUID付き仮メモを作って詳細シートを即時表示し、API作成完了後に同じIDの本物データへ差し替える。`生成` はAI整理追加の専用導線にし、純粋追加と分ける。クイック追加・AI整理から作ったメモも保存直後に同じ詳細シートを開く。メモ一覧取得と事前読み込みは `selectedSpaceId` だけでなく `selectedProjectId` も渡し、選択中プロジェクトへ追加したメモが同じ画面に即表示されるようにする。
