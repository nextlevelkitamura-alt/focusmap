 ...
 *   - DEEPSEEK_API_KEY 設定時: 速度優先で deepseek-v4-flash を使う
 *     (DEEPSEEK_INTENT_MODEL > DEEPSEEK_MODEL > デフォルト deepseek-v4-flash)
 *   - 未設定時: Gemini 3.1 Flash Lite にフォールバック
 *
 * ⚠️ 重要:
 * - deepseek-v4-pro は Thinking モードがデフォルトONで、
 *   Vercel AI SDK の generateObject (JSON schema 出力) と相性が悪く失敗する。
 *   intent判定は 軽量・高速で十分なので flash を使う。
 * - 実機検証済: deepseek-v4-flash は intent判定で 95%精度を達成
 *   (test-intent-classifier の出力確認、 2026-05-26)
 * - deepseek-chat / deepseek-reasoner は 2026-07-24 廃止予定 → v4-flash 推奨
 *
 * 自動化応答 (本格的なエージェント推論) は DEEPSEEK_AUTOMATION_MODEL (= deepseek-v4-pro)
 * を chat-workspace.tsx 側で別途使用する。
 */
function getModel() {
