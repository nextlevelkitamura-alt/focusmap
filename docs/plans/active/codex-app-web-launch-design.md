...
- スマホ判定時は `codex://` ではなくChatGPTアプリを直接狙う。iOS/iPadOSは公式Codex mobile入口の `https://chatgpt.com/codex/mobile/` を第一候補にし、Focusmap iOSアプリ内では `UIApplication.open(..., universalLinksOnly: true)` でChatGPTアプリがそのURLをCodex画面として扱える時だけ成功させる。fallback候補は `chatgpt://codex/mobile`、`chatgpt://codex`、`com.openai.chat://codex/mobile`、汎用 `chatgpt://` / `com.openai.chat://`。Android Chromeは `intent://chatgpt.com/codex/mobile/#Intent;scheme=https;package=com.openai.chatgpt;S.browser_fallback_url=https%3A%2F%2Fchatgpt.com%2Fcodex%2Fmobile%2F;end` でChatGPTアプリへ `chatgpt.com/codex/mobile/` を渡す。Focusmap iOSアプリ内WebViewではアンカー遷移ではなく、promptを `focusmap:copyText` bridgeでnative clipboardへ送り、Codex mobile URL候補を `focusmap:openExternal` bridgeでReact Native側へ渡す。iOS nativeのUniversal Link判定moduleは `mobile/focusmap-app/plugins/withFocusmapExternalOpener.js` がExpo prebuild時に生成するため、Webデプロイだけでは入らず、Focusmap iPhoneアプリの再インストールが必要。外部Webから任意のCodex画面へプロンプトを注入する公開deep linkや、Codexメニュー直行を保証する公開schemeは確認できないため、Focusmapは「コピー済み → 公式Codex URLのUniversal Linkを最優先で開く」までを保証範囲にする。
