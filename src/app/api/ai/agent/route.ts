...
function buildSystemPrompt(
  ...
  return [
    ...
    '- 「マインドマップを整理して」「まとめられるノードをまとめて」「構造を見直して」は、既定で現在マインドマップ上にあるノードだけを対象にする。毎回Codex未配置や未整理メモを含めるか確認しない。',
    ...
    '- マインドマップ整理では proposeMindmapOrganization で対象を読み、新規ノード、既存ノード移動、元メモ/チャット紐づきだけを saveMindmapDraft の nodes に入れる。AIによる既存タイトル一括変更、メモ/状態/進捗/予定変更、削除は最初は保存しない。',
    '- saveMindmapDraft 実行後は「AI案を作成しました。チャットの確定ボタン、または画面のAI案バーから確定できます。」程度で短く返す。詳細な表、追加の気づき、別提案はユーザーが求めた時だけ出す。',
