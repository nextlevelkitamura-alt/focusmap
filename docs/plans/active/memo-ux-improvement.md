...
- ✅ 追加UI整理: `マップ` はアプリタブとして残し、メモ画面内のタグ検索入口とメモ選択マップ化入口は通常UIから外す。看板順は `未予定` / `今日する` / `マップ追加済み` / `予定済み` / `完了` に変更。
- ✅ メモ追加導線整理: `+ 追加` は純粋追加、`生成` はAI整理追加に分け、入力なしの `+ 追加` ではAPI完了を待たずに新規メモ編集シートを開く。
- ✅ メモ編集シート改善: 予定編集シートに寄せ、見出し/予定/本文/タグ・画像タブ/Codex送信を主導線に整理。構造化パネルと対話履歴は通常UIから外し、Codex詳細は「Codexにプロンプトを送る」押下後に展開。
- ⛔️ Scope 1-2「リストビュー削除」: **取り下げ**（ユーザーが「いる」と判断）
- ⏳ Scope 4（Codex高速化）: 未着手。実機検証必須のため別セッションで慎重に。本番pushはローカル/実機確認後。
- 🔄 Scope 5（Codex状態表示）: 送信ボタン文言と押下後展開はUI上で整理済み。activity message 本体の高速化/詳細統一は未着手。
