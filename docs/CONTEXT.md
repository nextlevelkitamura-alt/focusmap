...
- モバイル自作マップのノードD&Dは、ノード上のタッチ開始時に長押し完了を待たずドラッグ候補を作り、移動量がしきい値を超えた時点でノード移動へ入る。ドラッグ中に指がviewport端のホットゾーンへ入ると、端に近いほど速く `panOffset` を更新してマップ表示を自動パンする。右/下端では先の領域を見せるためステージを左/上へ、左/上端ではステージを右/下へ動かす。自動パン中も最後のタッチ座標からdrag deltaとdrop対象を再計算し、指を止めたままでも配置先が更新される。実ドラッグ後のclickではモバイル編集シートを開かず、通常のタップ編集は維持する。
