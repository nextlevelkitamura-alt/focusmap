...
- `Todo` タブ左側のメモカードは、`メモ` 画面と同じ `WishlistCardDetail` 編集シートを開く。見出し・本文・タグ・画像・予定化などのメモ編集導線は左ペインからも同じ挙動にする。スマホのメモ編集シートは予定編集シートと同じ黒背景の下部シートに寄せ、上から見出し/プロジェクト、日付/時刻/所要時間、本文、タグ/画像の2タブ、保存、Codex送信の順に置く。構造化パネルと対話履歴は通常UIから外す。
- メモ編集シートの `画像` タブは、同じ添付API `/api/wishlist/[id]/attachments` を使い、クリックでフォルダーから選択、ドラッグ&ドロップ、`クリップボード画像を貼り付け` ボタン、画像タブ内の `Cmd/Ctrl+V` で画像を追加できる。サムネイルは操作ボタンより上に置き、アップロード中はローカルプレビューを低透明度で即時表示し、登録完了後に通常の濃さと保存/削除操作へ切り替える。複数画像は並列アップロードし、仮メモ作成中に追加された画像はメモ作成APIの完了を待ってから添付APIへ送る。画像アップロードは60秒でタイムアウトし、失敗時だけ対象の仮プレビューを消してエラーを表示する。画像削除はDELETE完了を待たずにサムネイルを即時非表示にし、失敗時だけ元の位置へ戻す。画像追加後も `画像` タブを開いたままにし、追加結果のサムネイル確認と削除を同じ場所で行う。添付画像のStorage保存パスはISO時刻トークンを使い、ミリ秒の巨大整数をDB値と誤認させない。
- `メモ` 画面の `+ 追加` は、入力欄に本文がある時はその先頭行をタイトルにして即時メモ追加し、入力欄が空の時だけ空の新規メモ詳細シートを開く。どちらもクライアント側で先にUUID付き仮メモを作って詳細シートを即時表示し、API作成完了後に同じIDの本物データへ差し替える。仮メモ作成中も詳細シートの保存ボタンは塞がず、編集内容は作成完了後の更新としてキューする。作成POSTは15秒タイムアウト付きでリトライし、サーバー側は同じクライアントUUIDの再送を既存メモ取得として扱う。これにより通信が固まった場合でも `作成中` 表示を無期限に残さず、再送で二重作成しない。仮メモの `display_order` はクライアントから作成APIへ送らず、サーバー側でPostgreSQL `integer` 範囲内の値だけを採用し、それ以外は件数ベースで採番する。`生成` はAI整理追加の専用導線にし、純粋追加と分ける。クイック追加・AI整理から作ったメモも保存直後に同じ詳細シートを開く。メモ一覧取得と事前読み込みは `selectedSpaceId` だけでなく `selectedProjectId` も渡し、選択中プロジェクトへ追加したメモが同じ画面に即表示されるようにする。
...
- Web側の `useMemoAiTasks` は、`running` のCodexタスクがある場合だけ3秒更新。確認待ちやプロンプト待ちは3秒監視せず、低頻度同期または手動更新で再開検知する。`useAiTasks` は既存どおり短周期表示を残すが、メモ実行の正は `useMemoAiTasks` / `/api/codex/sync-node` のrunning限定ポリシーに合わせる。
