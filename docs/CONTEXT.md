...
- `Today` の日/3日/月表示とヘッダー件数は、同じ `google_event_id` を持つ予定を1件として扱う。Google同期済みの手動タスクとカレンダー自動取り込みタスクが同時に存在しても、表示では手動タスクを優先し、月セルの `+N件` や月間件数を水増ししない。`/api/tasks/import-events` も既に同じ `google_event_id` の手動タスクがある場合は、新しい `source='google_event'` タスクを作らない。
