...
- Codex.app側でFocusmapを経由せず直接開始されたthreadは、Mac側 `focusmap-agent` が既存manual handoff taskの同期を先に試した後、`~/.codex/state_5.sqlite` の直近threadから未登録のものだけを検出して `/api/agents/codex-monitor/import-thread` へ送る。import API は `ai_tasks.codex_thread_id` / `ai_tasks.result.codex_thread_id` / `tasks.codex_thread_id` で冪等に重複を避け、repo path (`threads.cwd`) が一致するprojectを優先し、無ければ対象user/spaceの既存project、さらに無ければ `Codex Inbox: <repo>` projectを作る。取り込みノードはproject直下の `Codex Inbox` グループ配下に `source='codex_app_thread'` として作成し、同じthread idを `tasks.codex_thread_id` と `ai_tasks.codex_thread_id` に保存する。これにより、FocusmapからCodexへ送る既存manual handoff導線はそのまま維持しつつ、Codex.app起点の会話だけを未分類Inboxノードとして後から整理できる。
...
- Codex.appでFocusmapを経由せず直接開始したthreadは、Macの `focusmap-agent` が `~/.codex/state_5.sqlite` の最近の非archived/user-created threadを監視し、既存manual handoff taskに紐づかないものだけ `/api/agents/codex-monitor/import-thread` へ送って `Codex Inbox` ノードと `ai_tasks` を作る。これはWeb pushではなく、Macローカルagent/Electron supervisorとFocusmap APIの協調機能である。packaged通常起動のremote modeでは本番 `https://focusmap-official.com/api` にこのAPI routeが反映されていることが必須で、Cloud Run未反映なら取り込みは動かない。local mode / 開発起動では `http://127.0.0.1:3001/api` を使って同じagentを検証できる。agentは同API routeの404/405を検知したら数分backoffし、既存manual handoff同期は続けたまま直接開始threadの取り込みだけを一時停止する。設定の `Mac App Control` は `Codex thread取り込み` 診断として同API routeへtokenなしPOSTを送り、401/400系なら「対応済み」、404なら「未反映」と表示する。
