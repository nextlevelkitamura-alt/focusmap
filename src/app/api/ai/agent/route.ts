...
function buildSystemPrompt(
  ...
  return [
    ...
    '',
    '## ツールの種類',
    '- Focusmap DB確認/記録 (常に使える): listProjects / getProjectContext / saveProjectContext / updateProject / listProjectTasks / listNotesForOrganization / getNoteOrganizationDetail / bulkAddMemos',
    '- マインドマップDB操作 (常に使える): proposeMindmapOrganization / getMindmapOverview / getMindmapNodeDetail / updateMindmapNode / moveMindmapNode / updateMindmapMemoLink',
    ...
    '- 「記録して」「概要を更新して」「この進捗を残して」は saveProjectContext を使う。projects.description は安定した概要、project_contexts.details はAGENTS.md風の読みやすい背景メモ、project_contexts.progress は現在地・次の論点・ブロッカーの状況メモとして使う。プロジェクト名・状態・リポジトリなどプロジェクト本体を変更する時は updateProject を使う。',
    ...
    '- 「ノードを変更して」は updateMindmapNode、「このノードをここへ移して」「別プロジェクトに移して」は moveMindmapNode、「このメモの紐づきをこのノードへ変えて」は updateMindmapMemoLink を使う。',
    '- 「マインドマップを整理して」「まとめられるノードをまとめて」「構造を見直して」は、提案やツール実行の前に必ず「Codexから未配置/未取り込みのチャット・作業も含めるか」「現在マインドマップ上にあるノードだけで整理するか」を確認する。ユーザーが範囲を明示済みの場合だけ、その範囲で進める。',
    '- 範囲確認後、proposeMindmapOrganization を使う。Codex未配置/未取り込みも含める回答なら includeCodexInbox=true、マップ上だけなら includeCodexInbox=false かつ includeNoteHeadings=false にする。メモ/ノートも含めたいとユーザーが明示した場合だけ includeNoteHeadings=true にする。本文や詳細は必要な候補だけ getMindmapNodeDetail / getNoteOrganizationDetail で確認する。',
    '- マインドマップ整理では、DB変更前に「読んだ前提」「新規ノード作成案」「既存ノードへ入れる案」「移動対象」「理由」「実行操作」を短く示し、```text``` のツリー図で現在→提案後を見せる。ユーザー承認前に addMindmapGroup / moveMindmapNode / updateMindmapNode を実行しない。',
    '- ユーザーが整理案を承認したら、addMindmapGroup で必要なまとめノードを作り、返却された group.id を親にして moveMindmapNode を順番に使う。実行後は getMindmapOverview で結果を確認して報告する。',
    '- 「ノート/メモを整理して」は、まず listNotesForOrganization の detail_preview（通常30文字）で分類する。本文全量を無差別に読まず、判断に必要な候補だけ getNoteOrganizationDetail の detailChars を増やす。分類は「マップ紐づけ」「新規ノード/タスク化」「予定化」「保留」「却下」に分けて説明する。',
