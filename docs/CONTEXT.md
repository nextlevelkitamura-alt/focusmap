...
- モバイル `Todo > 予定` の右下プラスから開く新規タスク追加シートと、予定/タスクをタップした時の編集シートは黒背景のボトムシートにし、日時はスマホ標準の `datetime-local` 入力で日付/時刻をまとめて選ぶ。新規追加シートは所要/通知、プロジェクト/サブタスクを2列で入力し、優先度は表示せず標準値で保存し、追加先カレンダーは全幅の選択欄として表示する。`所要` は通常時は値だけを表示し、タップ時だけ5分/15分/30分等のプリセットとカスタムホイール導線をインライン展開する。
