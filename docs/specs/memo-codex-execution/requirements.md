...
- `今すぐ実行` は速さを優先し、本文をラベルや前置きなしでそのまま Codex に渡す。
...
- 画像は本文の補助素材として扱う。Codexへ送るprompt本文には添付画像ラベル、ファイル名、署名URLを同梱せず、保存済み画像ごとのコピー導線で同じCodex入力欄へ追加貼り付けする。
...
`今すぐ実行` の通常タップは、本文をそのまま Codex に渡す。本文が空ならタイトルだけを渡す。

```text
{body}
```

画像添付がある場合もprompt本文は上記の本文だけにし、画像名や署名URLはコピーしない。画像はUI上の画像コピー操作で個別にOSクリップボードへ保存する。
