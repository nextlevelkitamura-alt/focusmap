...
- 看板レーンは `実行中` / `確認待ち` / `接続失敗` / `完了`。`完了` はノードがチェック済みで、紐づくprogressの `updated_at` が当日のものだけを一時表示する。翌日以降は看板から消す。
...
- 実行中: `current_step`、`summary`、activity messages、詳細tailを上に出す。
- 確認待ち: Codex出力要約、確認待ち内容、追加入力の案内、activity messages、詳細tailを上に出す。
