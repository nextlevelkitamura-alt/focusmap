...
- メモ編集シートの `画像` タブは、同じ添付API `/api/wishlist/[id]/attachments` を使い、クリックでフォルダーから選択、ドラッグ&ドロップ、`クリップボード画像を貼り付け` ボタンの3経路で画像を追加できる。画像追加後も `画像` タブを開いたままにし、追加結果のサムネイル確認と削除を同じ場所で行う。
