...
- `saveMindmapDraft` でAI案を保存した直後のassistantメッセージには `metadata.focusmapMindmapDraftReady` を付け、`UnifiedChat` は同じチャット内に `AI案を確定` ボタンを出す。このボタンは画面上部のAI案バーと同じ `/api/mindmap/drafts/[draftId]/apply` を呼び、成功後は `focusmap:mindmap-draft:changed` を発火し、DB履歴ベースの `元に戻す` / `やり直す` 導線へつなぐ。AI案保存後のAI返答は短くし、詳細な表や追加の気づきはユーザーが求めた時だけ出す。
...
- `UnifiedChat` の通常送信は、画面のストリーミング接続を正にしない。送信直後に `agent_chat_sessions` へ user message と `status='running'` を保存し、`/api/ai/agent/runs` が Next `after()` で `/api/ai/agent` と同等のツール付きエージェント思考を継続して、完了時に assistant message と `completed` / 失敗時に `failed` を同じセッションへ保存する。背景実行中のツール開始/完了/失敗は、通常assistant本文ではなく `metadata.focusmapAgentProgress=true` の軽量progressメッセージとして `messages` にupsertし、ツール名ラベルだけを保存して入力値や出力全文は保存しない。チャット履歴と実行状態の正はSupabaseの `agent_chat_sessions` であり、`useAgentChatSessions` はlocalStorageを表示キャッシュとしてだけ使う。画面遷移、アプリ終了、復帰後はDBから履歴・`running` 状態・progressログ・完了結果を復元し、実行中セッションはSupabase Realtimeを優先、短周期pollをfallbackとして更新する。入力欄のマイク左にはChatGPT/Codex風の小さなモデルモードピルを置き、タップで `スピード` / `考える` を選ぶ。`考える` は既定で `DEEPSEEK_AGENT_MODEL` / `deepseek-v4-pro` を使い、論理整理・マインドマップ整理・複数ステップ実行を優先する。`スピード` は `DEEPSEEK_AGENT_SPEED_MODEL`、未設定なら `DEEPSEEK_INTENT_MODEL` / `deepseek-v4-flash` を使い、軽い確認や短い整理を速く返す。画像付きターンだけは従来通り Gemini vision 経路へ切り替える。AIモデルへ次の会話履歴を渡す時はprogressメッセージを除外し、ログ文言が会話文脈へ混ざらないようにする。Mac/ローカルファイル/Codex.appが必要なツール実行は引き続きMac agent接続状態に従い、クラウドだけでローカル権限を代替しない。
