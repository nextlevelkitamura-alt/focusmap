...
- ノード詳細を開いている間は、`未送信` / `実行中` / `確認待ち` のCodex taskだけ `/api/codex/sync-node` を約3秒ごとに呼び、Codex.appへの貼り付け・稼働開始・停止・最新のユーザー可視返信を短時間でUIへ反映する。CodexNodePanel、マップのリンクメモ詳細、Codex看板/右サイド詳細panel/drawerは開いた瞬間に `/api/task-progress/watch` へ `open`、表示中は10秒ごとに `ping`、閉じる時は `close` を送り、TTL 20秒程度のactive watchとして扱う。チャット本文を回収する `/api/codex/sync-node` 呼び出しは `include_visible_activity=true` を付け、sync後に `/api/ai-tasks/[id]/activity` を読む。マップ全体や `useMemoAiTasks` の軽量状態同期はこのフラグを付けず、返信本文は保存しない。`/api/codex/sync-node` は、thread検出、状態変化、Codex thread `updated_at_ms` / `last_activity_at` 変化、current_step変化、一定間隔の `codex_last_checked_at` 更新がある時だけSupabase/Tursoへ書き、同じsnapshotを3秒ごとに保存しない。
...
- ローカル同期が使える環境では `scripts/task-runner.ts` または `/api/codex/sync-node` が `~/.codex/state_5.sqlite` と rollout JSONL を読み、`ai_tasks.result` に状態を同期する。手動実行の起動自体はこの常駐同期を前提にしない。
...
- メモからCodexへ渡す導線と活動表示の詳細計画は `docs/specs/memo-codex-execution/requirements.md` / `delivery-plan.md` を参考にするが、標準UIはmanual handoffを正とする。確認待ち後にCodex側で追加プロンプトが送られた場合は、UI詳細表示中は約3秒、常駐runner単体でも5秒以内を目安に再開を検知して `running` snapshot/eventへ戻す。
...
- Macアプリは「Focusmap 接続状態」ウィンドウを持たず、起動時はメインウィンドウのローディング画面から直接 `/dashboard?desktop=1&source=mac` へ遷移する。メニューからは既存 `scripts/focusmap-agent/dist/cli.js` と `scripts/run-codex-app-server.sh` の起動だけを実行できる。agentの設定は従来通り `~/.focusmap/config.json` を使い、Macアプリ内にservice role key等は置かない。
...
- Macアプリ/ローカルWebのメモ詳細では、Codex.appへの自動 `thread/start` / `turn/start` を標準導線にしない。Focusmapはユーザー本文だけのprompt / metadata上のhandoff token / ai_task tracking packageを作り、クリップボードコピーとCodex.app起動まで補助する。人間がCodex.appで送信した後、Electron/常駐runner側が `ANTHROPIC_API_KEY` / `CLAUDECODE` を外した環境でローカルsqlite/rollout/app-serverを監視し、Turso snapshot/eventへ同期する。明示 `dispatch_mode='auto'` の互換経路は残すが、通常UIはmanual handoffを正とする。
